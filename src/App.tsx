...
function P3({onSave}){
  ...
  const files=[f0,f1,f2];const names=[n0,n1,n2];
  const setFiles=[setF0,setF1,setF2];const setNames=[setN0,setN1,setN2];const refs=[r0,r1,r2];
  const handle=(idx,e)=>{const f=e.target.files&&e.target.files[0];if(!f)return;const url=URL.createObjectURL(f);setFiles[idx](url);if(onSave)onSave({id:Date.now()+Math.random(),name:names[idx]||f.name,type:f.type,file:f,url});};
  const inpSt={width:"100%",background:"#000",border:`1px solid ${GOLDDIM}`,padding:"9px 12px",color:WHITE,fontSize:13,fontWeight:700,outline:"none",fontFamily:"'Rajdhani',sans-serif",letterSpacing:1,boxSizing:"border-box",marginBottom:6};
  const btnSt={display:"block",width:"100%",padding:"10px",background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`,color:"#000",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",textAlign:"center",cursor:"pointer",border:"none",outline:"none"};
  const slot=(idx)=>(<div style={{...Card(),padding:0,overflow:"hidden"}}><div style={{background:"#000",aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",borderBottom:`1px solid ${GOLDDIM}`}}>{files[idx]?<video src={files[idx]} controls playsInline style={{width:"100%",height:"100%",objectFit:"contain"}}/>:<div style={{textAlign:"center"}}><div style={{fontSize:40}}>🎬</div><div style={{color:GOLDDIM,fontSize:11,marginTop:6,letterSpacing:2}}>PROOF OF CONCEPT</div><div style={{color:DIM,fontSize:10,marginTop:3}}>SLOT {idx+1}</div></div>}</div><div style={{padding:"10px 12px 12px"}}><input value={names[idx]} onChange={e=>setNames[idx](e.target.value)} placeholder="Enter film title..." style={inpSt}/><input ref={refs[idx]} type="file" accept="video/*" style={{display:"none"}} onChange={e=>handle(idx,e)}/><button onClick={()=>refs[idx].current&&refs[idx].current.click()} style={btnSt}>{files[idx]?"CHANGE VIDEO":"UPLOAD VIDEO"}</button></div></div>);
  return(<div style={{...Sp,padding:40}}><div style={{maxWidth:980,margin:"0 auto"}}><div style={{fontSize:12,color:GOLD,letterSpacing:4,marginBottom:4,fontWeight:700}}>SHOWCASE</div><h1 style={{...H1,fontSize:30,marginBottom:8}}>MADE WITH MANDASTRONG STUDIO</h1><div style={{color:WHITE,fontSize:13,lineHeight:1.8,marginBottom:24}}>Real films produced entirely inside this platform. Click play to watch.</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>{slot(0)}{slot(1)}{slot(2)}</div></div></div>);
}

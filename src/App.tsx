...
// MANDASTRONG ENGINE â€” real photorealistic footage
...
// â”€â”€ CINEMA VOICE ENGINE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Server-side speech. Same voice on every device â€” iPad, Galaxy, HP.
...
// â”€â”€ HIDDEN: mint a personal cloned voice from a sample recording â”€â”€
...
// Health check â€” tells you if the engine has a provider key installed.
...
// â”€â”€ Background storage manager â€” prevents the save-crash on low-memory machines â”€â”€
...
// Guarded save â€” frees space first if storage is nearly full, then saves. Never silently crashes.
...
// â”€â”€ BACKGROUND STORAGE GUARD â€” prevents the save-crash automatically â”€â”€
...
const autoFreeStorage=async()=>{
  try{
    ...
    const clips=await getAllClipsFromDB();
    // oldest first â€” ids that start with a timestamp sort naturally; fall back to insertion order
...
const STOCK_VOICES = [
  { id:"aurora", name:"Aurora", desc:"Warm British Female", style:"Documentary Â· Narrator", accent:"British RP" },
  { id:"marcus", name:"Marcus", desc:"Deep American Male", style:"Cinematic Â· Authoritative", accent:"American" },
  { id:"sophia", name:"Sophia", desc:"Bright Australian Female", style:"Upbeat Â· Engaging", accent:"Australian" },
  { id:"james",  name:"James",  desc:"Dry British Male", style:"Sarcastic Â· Witty", accent:"British" },
  { id:"nova",   name:"Nova",   desc:"Neutral AI Female", style:"Clean Â· Professional", accent:"Neutral" },
  { id:"river",  name:"River",  desc:"Warm American Male", style:"Friendly Â· Intimate", accent:"American South" },
];
...
function speakText(voiceId, txt, onStart, onEnd) {
  ...
  const doSpeak = () => {
    ...
    // â”€â”€ QUALITY FIRST: always prefer the highest-quality voice the device has â”€â”€
    ...
    for(const name of candidates){
      ...
      // Nothing matched by name â€” take the best-quality voice matching gender/accent
    ...
    // Final fallbacks â€” still prefer quality
    if(!picked){
      ...
      <div style={{width:"min(620px,95vw)",background:"#050505",border:"2px solid "+GOLD,maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:18}}>
          {filtered.length===0?(
            <div style={{textAlign:"center",padding:"40px 20px",color:GOLDDIM}}>
              ...
              <div style={{fontSize:11,color:DIM,lineHeight:1.7}}>{tab==="in_progress"?<span>Hit ðŸ’¾ SAVE PROJECT with<br/>status IN PROGRESS to save your work.</span>:<span>Mark a project COMPLETED<br/>when your film is finished.</span>}</div>
            </div>
            ...
            return (
              <div key={i} style={{background:"#0a0a0a",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{color:DIM,fontSize:10,letterSpacing:1}}>{h.date} Â· Page {h.page} Â· {h.assetCount} asset{h.assetCount!==1?"s":""}</div>
                  {h.note&&<div style={{color:WHITE,fontSize:11,marginTop:4,fontStyle:"italic"}}>{h.note}</div>}
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button onClick={()=>onResume(h)} style={{background:"linear-gradient(135deg,#a07820,#e8c96d)",border:"none",color:"#000",padding:"8px 18px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>{tab==="completed"?"ðŸ‘ REVISIT":"â–¶ CONTINUE"}</button>
                  <button onClick={()=>del(originalIdx)} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"5px 10px",cursor:"pointer",fontSize:10,fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>âœ•</button>
                </div>
              </div>
            );
          })}
        ...
        {history.length>0&&(
          <div style={{borderTop:"1px solid "+GOLDDIM+"",padding:"10px 18px",flexShrink:0}}>
            <button onClick={()=>{if(confirm("Delete all project history?")){{localStorage.removeItem("ms_project_history");setHistory([]);}}}} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"5px 14px",cursor:"pointer",fontSize:10,fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>ðŸ—‘ CLEAR ALL</button>
          </div>
        )}
      </div>
    </div>
...
function SaveSessionModal({ onClose, onSave, currentPage, assetCount }) {
  const [name,setName]=useState("Session â€” "+new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}));
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1200,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(440px,92vw)",background:"#050505",border:"2px solid "+GOLD,padding:22}}>
        <div style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:15,fontWeight:900,letterSpacing:3,marginBottom:4}}>ðŸ’¾ SAVE SESSION</div>
        <div style={{color:DIM,fontSize:10,marginBottom:14}}>Page {currentPage} Â· {assetCount} assets in library</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:16}}>
          <button onClick={()=>setStatus("in_progress")} style={{background:status==="in_progress"?GOLD:"#111",border:"1px solid "+(status==="in_progress"?"#000":GOLDDIM),color:status==="in_progress"?"#000":WHITE,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>âŸ³ IN PROGRESS</button>
          <button onClick={()=>setStatus("completed")} style={{background:status==="completed"?GOLD:"#111",border:"1px solid "+(status==="completed"?"#000":GOLDDIM),color:status==="completed"?"#000":WHITE,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>âœ“ COMPLETED</button>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <button onClick={()=>onSave(name,note,status)} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"11px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>ðŸ’¾ SAVE</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex"}}>
      <div style={{width:256,background:"#050505",borderRight:"1px solid "+GOLD+"",height:"100vh",overflowY:"auto",padding:18}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:GOLD,fontSize:20,cursor:"pointer"}}>âœ•</button>
        </div>
...
function Header({ go, setMenu }) {
  return (
    <header style={{position:"sticky",top:0,zIndex:500,background:"#000",borderBottom:"1px solid "+GOLD+"",padding:"0 16px",height:52,display:"flex",alignItems:"center",gap:12}}>
      <button onClick={()=>setMenu(true)} style={{background:"none",border:"1px solid "+GOLD,color:GOLD,width:34,height:34,cursor:"pointer",fontSize:16,flexShrink:0}}>â˜°</button>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{color:GOLD,fontSize:11,letterSpacing:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontWeight:700}}>
          âœ¦ CINEMA INTELLIGENCE PLATFORM &nbsp;Â·&nbsp; 600+ AI TOOLS &nbsp;Â·&nbsp; 8K EXPORT &nbsp;Â·&nbsp; UP TO 3-HOUR FILMS
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{color:"#22c55e",fontSize:11,letterSpacing:2,fontWeight:900}}>â— SYSTEM ONLINE</div>
  ...
  return (
    <footer style={{position:"fixed",bottom:0,left:0,right:0,zIndex:400,background:"#000",borderTop:"1px solid "+GOLD+"",padding:"6px 20px 8px",display:"flex",flexDirection:"column",gap:4}}>
      <div style={{textAlign:"center"}}>
        <span style={{color:GOLD,fontSize:11,letterSpacing:1,fontWeight:700}}>MANDASTRONG STUDIO Â· PROFESSIONAL CINEMA SYNTHESIS Â· MandaStrong1.Etsy.com</span>
        {page===1&&<span style={{color:GOLD,fontSize:11,letterSpacing:1,fontWeight:700,opacity:0.75}}> Â· CREATED 2025</span>}
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
        <button onClick={()=>go(Math.max(1,page-1))} disabled={page===1} style={{...G("out",true),opacity:page===1?0.3:1}}>â—€ BACK</button>
        <span style={{color:GOLD,fontSize:11,fontWeight:900,fontFamily:"'Cinzel',serif",letterSpacing:2}}>PAGE {page} / {TOTAL}</span>
        <button onClick={()=>go(Math.min(TOTAL,page+1))} disabled={page===TOTAL} style={{...G("gold",true),opacity:page===TOTAL?0.3:1}}>NEXT â–¶</button>
        <button onClick={onSave} style={{...G("out",true),fontSize:11,letterSpacing:2}}>ðŸ’¾ SAVE PROJECT</button>
        <button onClick={onHistory} style={{background:"linear-gradient(135deg,#0a0300,#1a0800)",border:"1px solid "+GOLD,color:GOLD,padding:"5px 14px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>ðŸ“‚ MY PROJECTS</button>
        <span style={{color:"#22c55e",fontSize:11,fontWeight:700}}>â— AUTOSAVE ON</span>
      </div>
    </footer>
  );
}
...
function ToolPanel({ tool, onClose, onSave }) {
  ...
  const runAI = async () => {
    ...
    } catch(e) { setResult("Error â€” check your connection and try again."); }
    setLoading(false);
  };
  ...
  const saveAsset = () => {
    ...
    if (onSave) onSave({id:Date.now()+Math.random(),name:tool+" â€” "+isVoice?STOCK_VOICES.find(x=>x.id===selVoice)?.name:"Result",type:isVoice?"audio/narration":"text/plain",url:"",content});
    setSaved(true);
  };
    ...
    <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(600px,95vw)",background:"#050505",border:"1px solid "+GOLD,padding:26,maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isVoice?"1fr 1fr 1fr 1fr":"1fr 1fr 1fr",gap:8,marginBottom:18}}>
          {isVoice&&<button onClick={()=>setMode("voice")} style={{...G(mode==="voice"?"gold":"out",true),fontSize:11}}>ðŸŽ™ VOICE</button>}
          {[["upload","UPLOAD"],["paste","PASTE"],["ai","AI CREATE âœ¦"]].map(([m,l])=>(
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              {STOCK_VOICES.map(v=>(
                <div key={v.id} onClick={()=>setSelVoice(v.id)}
                  ...
                  <div style={{color:WHITE,fontSize:10,marginTop:2}}>{v.style} Â· {v.accent}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:result?14:0}}>
              <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),padding:"12px",opacity:loading||!describe.trim()?0.5:1}}>
                {loading?"âŸ³ GENERATING...":"AI FORMAT & SPEAK âœ¦"}
              </button>
            ...
            {result&&(
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                  <button onClick={()=>speak(selVoice,result)} style={{...G("out",false),padding:"10px"}}>â–¶ PLAY</button>
                  <button onClick={stopSpeaking} style={{...G("out",false),padding:"10px"}}>â¹ STOP</button>
        ...
        {mode==="upload"&&(
          <div style={{marginBottom:14}}>
            ...
            <a href="https://photos.google.com" target="_blank" rel="noopener noreferrer"
              ...
              ðŸŒ OPEN GOOGLE PHOTOS â†’ download photo â†’ then Upload Photo above
            </a>
        ...
        {mode==="ai"&&tool==="Script to Movie"&&(
          <div style={{marginBottom:14}}>
            <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:2}}>ðŸŽ¬ SCRIPT TO MOVIE</div>
            <div style={{color:GOLDDIM,fontSize:11,lineHeight:1.6,marginBottom:12}}>Fill the three boxes, then WIRE INTO RENDER â€” the video generator on Page 8 uses them to drive every scene.</div>
            <div style={{color:GOLD,fontSize:11,letterSpacing:2,fontWeight:900,marginBottom:3}}>ðŸŽ¬ PRODUCER</div>
            <textarea value={s2mProducer} onChange={e=>setS2mProducer(e.target.value)} placeholder="Producer's directions â€” vision, mood, casting, overall intent..." style={{...inp,height:80,resize:"vertical",lineHeight:1.6,marginBottom:10}}/>
            <div style={{color:GOLD,fontSize:11,letterSpacing:2,fontWeight:900,marginBottom:3}}>ðŸŽž DESCRIBE</div>
            <textarea value={describe} onChange={e=>setDescribe(e.target.value)} placeholder="Describe your film â€” scene by scene, what happens on screen..." style={{...inp,height:80,resize:"vertical",lineHeight:1.6,marginBottom:10}}/>
            <div style={{color:GOLD,fontSize:11,letterSpacing:2,fontWeight:900,marginBottom:3}}>ðŸŽ¥ PRODUCTION NOTES</div>
            <textarea value={s2mProduction} onChange={e=>setS2mProduction(e.target.value)} placeholder="Production notes â€” shots, camera moves, lighting, locations, timing..." style={{...inp,height:80,resize:"vertical",lineHeight:1.6,marginBottom:10}}/>
            <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),width:"100%",padding:"13px",opacity:loading||!describe.trim()?0.5:1,fontSize:13,letterSpacing:2,marginBottom:8}}>{loading?"âŸ³ CREATING...":"âœ WRITE SCRIPT âœ¦"}</button>
            <button onClick={()=>{
              ...
              if(onSave)onSave({id,name:"SCRIPT-TO-MOVIE BRIEF â€” "+new Date().toLocaleDateString(),type:"document",docKind:"brief",text:brief,date:new Date().toISOString(),renderBrief:true});
              setS2mWired(true);setTimeout(()=>setS2mWired(false),4000);
            }} style={{width:"100%",background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"14px",cursor:"pointer",fontSize:13,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>âš¡ WIRE INTO RENDER â€” DRIVE THE VIDEO GENERATOR</button>
            {s2mWired&&<div style={{color:"#22c55e",fontSize:11,fontWeight:900,letterSpacing:1,marginTop:10,textAlign:"center"}}>âœ“ WIRED â€” Page 8 will use your Producer, Describe &amp; Production notes.</div>}
            {result&&(
        ...
        {mode==="ai"&&tool!=="Script to Movie"&&(
          <div style={{marginBottom:14}}>
            ...
            <button onClick={runAI} disabled={loading||!describe.trim()} style={{...G("gold",false),marginTop:8,width:"100%",padding:"14px",opacity:loading||!describe.trim()?0.5:1,fontSize:13,letterSpacing:2}}>
              {loading?"âŸ³ CREATING...":isVideoTool?"ðŸŽ¬ CREATE VIDEO PACKAGE âœ¦":isImageTool?"ðŸŽ¨ CREATE IMAGE PROMPT âœ¦":isWritingTool?"âœ WRITE SCRIPT âœ¦":"âœ¦ AI CREATE"}
            </button>
            {result&&(
              <div style={{marginTop:14}}>
                <textarea value={result} onChange={e=>setResult(e.target.value)} style={{...inp,height:140,resize:"none",lineHeight:1.7}}/>
        ...
        {saved&&(
          <div style={{marginTop:14,background:"#0a2a0a",border:"1px solid #22c55e",padding:"12px 16px",textAlign:"center"}}>
            <div style={{color:"#22c55e",fontWeight:900,fontSize:14,letterSpacing:2}}>âœ“ ASSET SAVED TO MEDIA LIBRARY</div>
          </div>
        )}
      </div>
    </div>
...
// â”€â”€ PAGE 5 WRITING BOXES â€” Producer Directions / Script / Production Directions
function WritingBoxes({ onSave }) {
  const BOXES=[
    {key:"producer",title:"PRODUCER",icon:"ðŸŽ¬",hint:"Vision, tone, casting, the feeling the film should leave behind.",ph:"Producer's directions â€” vision, mood, casting, overall intent..."},
    {key:"describe",title:"DESCRIBE",icon:"ðŸŽž",hint:"Describe the film scene by scene â€” what happens, what we see.",ph:"Describe your film â€” scene by scene, what happens on screen..."},
    {key:"production",title:"PRODUCTION NOTES",icon:"ðŸŽ¥",hint:"Shots, camera moves, lighting, locations, timing.",ph:"Production notes â€” shots, camera moves, lighting, locations, timing..."},
  ];
  ...
  const saveBox=async(b)=>{
    ...
    const asset={id,name:b.title+" â€” "+new Date().toLocaleDateString(),type:"document",docKind:b.key,text:body,date:new Date().toISOString()};
  ...
  // â”€â”€ WIRE INTO RENDER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const wireToRender=async()=>{
    ...
    const id="brief_"+Date.now();
    const asset={id,name:"SCRIPT-TO-MOVIE BRIEF â€” "+new Date().toLocaleDateString(),type:"document",docKind:"brief",text:brief,date:new Date().toISOString(),renderBrief:true};
  ...
  return (
    <div style={{padding:"0 12px 16px"}}>
      <div style={{color:GOLD,fontSize:13,letterSpacing:3,fontWeight:900,margin:"6px 2px 4px"}}>ðŸŽ¬ SCRIPT TO MOVIE</div>
      <div style={{color:GOLDDIM,fontSize:11,letterSpacing:1,margin:"0 2px 12px"}}>Fill the three boxes, then WIRE INTO RENDER â€” the video generator on Page 8 uses them to drive every scene. Each box also saves to your Media Library &amp; timeline.</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12}}>
        {BOXES.map(b=>(
          <div key={b.key} style={{background:"#050500",border:"2px solid "+GOLD,padding:"14px 16px",display:"flex",flexDirection:"column"}}>
            ...
            <button onClick={()=>saveBox(b)} style={{marginTop:10,background:"transparent",border:"1px solid "+GOLD,color:GOLD,padding:"9px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>ðŸ’¾ SAVE TO MEDIA LIBRARY</button>
            {saved===b.key&&<div style={{color:"#22c55e",fontSize:11,fontWeight:900,letterSpacing:1,marginTop:8,textAlign:"center"}}>âœ“ SAVED</div>}
          </div>
        ))}
      </div>
      <button onClick={wireToRender} disabled={wiring} style={{marginTop:14,width:"100%",background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"15px",cursor:"pointer",fontSize:14,fontWeight:900,letterSpacing:3,fontFamily:"'Rajdhani',sans-serif"}}>{wiring?"âŸ³ WIRING...":"âš¡ WIRE INTO RENDER â€” DRIVE THE VIDEO GENERATOR"}</button>
      {wired&&<div style={{color:"#22c55e",fontSize:12,fontWeight:900,letterSpacing:1,marginTop:10,textAlign:"center"}}>âœ“ WIRED â€” Page 8 will now use your Producer, Describe &amp; Production notes on every scene.</div>}
    </div>
  );
}
      ...
      <div style={{padding:"14px 18px 12px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{position:"relative"}}>
            ...
            <span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",color:GOLD}}>ðŸ”</span>
            {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:7,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:GOLD,cursor:"pointer",padding:0}}>âœ•</button>}
          </div>
      ...
      {title==="WRITING TOOLS"&&(
        <div style={{padding:"0 12px 12px"}}>
          <div style={{background:"#050500",border:"2px solid "+GOLD,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
            <div>
              <div style={{color:GOLD,fontWeight:900,fontSize:13,letterSpacing:3}}>ðŸ“‚ YOUR PROJECTS</div>
              ...
              <button onClick={()=>{
                try{
                  ...
                  if(hist.length>0){
                    ...
                    alert("No saved projects found. Hit ðŸ’¾ SAVE PROJECT in the footer to save your work.");
                  }
...
function RecordYourOwnSong({ onRecorded }) {
  ...
  return recording?(
    <div style={{display:"flex",alignItems:"center",gap:10,background:"#1a0000",border:"1px solid #ef4444",padding:"10px 14px",marginTop:8}}>
      ...
      <span style={{color:"#ef4444",fontWeight:900,fontSize:12,letterSpacing:2,flex:1}}>RECORDING â€” {fmt(recTime)}</span>
      <button onClick={stop} style={{background:"#ef4444",border:"none",color:"#fff",padding:"6px 16px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>â–  STOP & SAVE</button>
    </div>
  ):(
...
function MusicVideoStudio({ onClose, onSave }) {
  ...
  const generateVideo = async () => {
    ...
    try {
      ...
      addLog("MandaStrong Cinema Engine â€” writing your film...");
      setRenderProgress(4);

      // â”€â”€ BEAT ANALYSIS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      // â”€â”€ DURATION â”€â”€ single source of truth is the slider (config.durationMin).
      ...
      else { addLog("Duration: AUTO â€” will match the song length"); }
      let beatGrid = [];
      ...
      if(audioFile){
        try{
          ...
          addLog("Audio: "+totalDur.toFixed(1)+"s â€” "+beatGrid.length+" beats detected");
          // Set up audio mixing
          ...
          if(config.stereo!==false && audioCtx.createStereoPanner){
            addLog("ðŸ”Š Stereo sound ON â€” baking full stereo field into the film");
            ...
            addLog("Mono sound â€” stereo toggle off");
            audioSource.connect(gain); gain.connect(audioDest); gain.connect(audioCtx.destination);
          }
          if(overrideSec>0){ totalDur = overrideSec; } else { totalDur = buf.duration; }
        }catch(e){ addLog("Audio: "+e.message); audioCtx=null; }
      } else {
        addLog("No audio â€” generating "+totalDur+"s visual");
        for(let t=0;t<totalDur;t+=1.8) beatGrid.push(t);
      }
      setRenderProgress(10);

      // â”€â”€ BUILT-IN RENDERER (NO PROXY) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      addLog("MandaStrong Engine â€” built-in renderer ready");
      ...
      // Load reference image if user uploaded one â€” Reality Engine base
      let refImgEl = null;
      if(config.refMedia){
        try{
          ...
          if(refImgEl) addLog("âœ“ Reference image loaded â€” Reality Engine base active");
        }catch(e){}
      }
      ...
      const renderFn = (ctx, W, H, t, sec, totalSec, beatNow) => {
        ...
        // â”€â”€ REALITY ENGINE â€” if user uploaded a reference image, use it as photorealistic base
        if(refImgEl){
      ...
      // MANDASTRONG ENGINE â€” real footage, not drawn shapes
      ...
      // â”€â”€ SET UP CANVAS + RECORDER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // Start audio at exact same moment as video recording â€” guarantees sync
      if(audioSource&&audioCtx) audioSource.start(audioCtx.currentTime);
      else if(audioSource) audioSource.start(0);

      // â”€â”€ RENDER EVERY FRAME â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      await new Promise(resolve=>{
        ...
        const tick=()=>{
          ...
          try{
            ...
            // Graceful fallback â€” keep rendering
          ...
          // â”€â”€ AUTO-ENHANCEMENT â€” runs every frame automatically â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          // Warm gold colour grade overlay
          ctx.fillStyle="rgba(232,180,60,0.06)";ctx.fillRect(0,0,W,H);
          // Contrast boost â€” darken shadows slightly
          ctx.fillStyle="rgba(0,0,0,0.08)";ctx.fillRect(0,0,W,H);
          // Highlight recovery â€” soft white pull on bright areas (top centre)
          ...
          // Title card â€” opening and closing
          if(t<0.12||t>0.9){
      ...
      // â”€â”€ FINALISE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      addLog("âœ“ "+config.title+" complete â€” "+(blob.size/1024/1024).toFixed(1)+"MB Â· "+Math.round(totalDur)+"s");
  ...
  const steps = ["ðŸŽµ SONG","ðŸŽ¤ STYLE","ðŸŽ¬ SCENE","â–¶ GENERATE"];
    ...
    <div style={{position:"fixed",inset:0,zIndex:1100,background:"rgba(0,0,0,0.98)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(960px,98vw)",height:"min(92vh,860px)",background:"#050505",border:"2px solid "+GOLD,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#1a0a00,#0a0500)",borderBottom:"1px solid "+GOLD+"",padding:"14px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
            <div style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:18,fontWeight:900,letterSpacing:4}}>ðŸŽ¬ MUSIC VIDEO STUDIO</div>
            <div style={{color:WHITE,fontSize:10,letterSpacing:3,marginTop:2}}>PROFESSIONAL MUSIC VIDEO PRODUCTION Â· AI POWERED Â· SELF-CONTAINED</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"1px solid "+GOLD,color:GOLD,width:32,height:32,cursor:"pointer",fontSize:16}}>âœ•</button>
        </div>
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:videoUrl?"1fr 1fr":"1fr",overflow:"hidden"}}>
          ...
          <div style={{overflowY:"auto",padding:"16px 20px",borderRight:videoUrl?"1px solid "+GOLDDIM:"none"}}>
            ...
            {step===1&&(
              <div>
                ...
                {label("â¬† UPLOAD YOUR SONG â€” OR RECORD IT BELOW")}
                <div style={{background:"#000",border:"2px dashed "+(audioFile?GOLD:GOLDDIM),padding:"18px 12px",cursor:"pointer",transition:"all .2s"}}
                  ...
                  onDrop={e=>{
                    ...
                    else if(f){alert("Please drop an audio file â€” MP3, WAV or M4A.");}
                  }}>
                  <div style={{color:audioFile?"#22c55e":GOLD,fontWeight:900,fontSize:13,letterSpacing:2,textAlign:"center"}}>
                    {audioFile?"âœ“ "+audioName:"â¬† DRAG & DROP YOUR SONG HERE"}
                  </div>
                  <div style={{color:GOLDDIM,fontSize:10,marginTop:4,textAlign:"center",letterSpacing:1}}>{audioFile?"Tap to replace":"or tap to browse â€” MP3 Â· WAV Â· M4A"}</div>
                </div>
                ...
                {audioFile&&<button onClick={()=>{setAudioFile(null);setAudioUrl("");setAudioName("");}} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"3px 10px",cursor:"pointer",fontSize:10,fontWeight:900,marginTop:6}}>âœ• REMOVE AUDIO</button>}
                <div onClick={()=>set("stereo",!config.stereo)} style={{display:"flex",alignItems:"center",gap:10,marginTop:14,padding:"10px 12px",background:"#0a0a0a",border:"1px solid "+(config.stereo?GOLD:GOLDDIM),cursor:"pointer"}}>
                  <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(config.stereo?GOLD:GOLDDIM),background:config.stereo?GOLD:"transparent",color:"#000",fontWeight:900,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{config.stereo?"âœ“":""}</div>
                  <div><div style={{color:config.stereo?GOLD:WHITE,fontWeight:900,fontSize:12,letterSpacing:1}}>ðŸ”Š USE STEREO SOUND</div><div style={{color:GOLDDIM,fontSize:10,marginTop:1}}>Full stereo width baked into the exported video</div></div>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div style={{padding:"12px 14px",border:"1px solid "+GOLD,background:"#0a0a0a",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    ...
                    <span style={{color:GOLD,fontSize:13,fontWeight:900}}>{config.durationMin>0?(config.durationMin+" min"):"AUTO â€” match song"}</span>
                  </div>
                  ...
                  <div style={{color:config.durationMin>0?GOLD:GOLDDIM,fontSize:11,lineHeight:1.7,marginTop:8}}>
                    {config.durationMin>0
                      ? "â± This overrides the song length â€” the film will run exactly "+config.durationMin+" minute"+(config.durationMin===1?"":"s")+", looping or trimming the audio to fit."
                      : "ðŸŽµ At 0 the video automatically matches your song's length. Drag right to set a fixed length (up to 180 minutes) that overrides the music."}
                  </div>
                </div>
              </div>
            )}
            ...
            {step===4&&(
              <div>
                ...
                {/* Drag-drop audio upload â€” moved here from Step 1 */}
                {label("â¬† UPLOAD YOUR AUDIO TRACK")}
                <div style={{background:"#000",border:"2px dashed "+(audioFile?GOLD:GOLDDIM),padding:"16px 12px",cursor:"pointer",marginBottom:8,transition:"border-color .2s"}}
                  ...
                  <div style={{color:audioFile?"#22c55e":WHITE,fontWeight:900,fontSize:12,letterSpacing:2,textAlign:"center"}}>
                    {audioFile?"âœ“ "+audioName:"â¬† DRAG & DROP or CLICK â€” MP3 / WAV / M4A"}
                  </div>
                  {audioFile&&<div style={{color:GOLDDIM,fontSize:10,marginTop:4,textAlign:"center"}}>Audio will sync with your video â€” starts and ends together</div>}
                </div>
                <input ref={audioInputRef} type="file" accept="audio/*" style={{display:"none"}} onChange={handleAudioUpload}/>
                {audioFile&&<button onClick={()=>{setAudioFile(null);setAudioUrl("");setAudioName("");}} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"3px 10px",cursor:"pointer",fontSize:10,fontWeight:900,marginBottom:8}}>âœ• REMOVE AUDIO</button>}
                ...
                {label("â¬† UPLOAD REFERENCE IMAGE (OPTIONAL)")}
                {config.refMedia?(
                  <div style={{position:"relative",marginBottom:10}}>
                    <img src={config.refMedia} alt="ref" style={{width:"100%",height:70,objectFit:"cover",border:"1px solid "+GOLD}}/>
                    <button onClick={()=>set("refMedia",null)} style={{position:"absolute",top:4,right:4,background:"#000",border:"1px solid "+GOLD,color:GOLD,padding:"1px 7px",cursor:"pointer",fontSize:10,fontWeight:900}}>âœ•</button>
                    <div style={{color:"#22c55e",fontSize:9,fontWeight:900,letterSpacing:2,marginTop:3}}>âœ“ REFERENCE LOADED</div>
                  </div>
                ):(
                  <div>
                    <div
                      ...
                      <div style={{color:GOLD,fontSize:14,fontWeight:900,letterSpacing:2,marginBottom:4}}>â¬† DRAG & DROP HERE</div>
                      <div style={{color:GOLDDIM,fontSize:10,letterSpacing:2}}>or click to browse â€” JPG Â· PNG Â· MP4</div>
                    </div>
                ...
                <div style={{background:"#0a0500",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
                  ...
                  {[["TITLE",config.title||"â€”"],["ARTIST",config.artist||"â€”"],["GENRE",config.genre||"â€”"],["MOOD",config.mood||"â€”"],["STYLE",config.videoStyle||"â€”"],["GRADE",config.colorGrade||"â€”"],["DURATION",config.durationMin>0?(config.durationMin+" min"):"Auto â€” match song"],["AUDIO",audioName||"No audio uploaded"]].map(([k,v])=>(
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  <button onClick={()=>set("lipSync",!config.lipSync)}
                    ...
                    {config.lipSync?"âœ“ AUTO LIP SYNC ON":"AUTO LIP SYNC"}
                  </button>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
                  <span style={{color:"#22c55e",fontSize:11,fontWeight:900}}>â— AUTOSAVE ON</span>
                ...
                <button onClick={generateVideo} disabled={generating}
                  ...
                  {generating?"âŸ³ RENDERING... "+renderProgress+"%":"ðŸŽ¬ GENERATE MUSIC VIDEO"}
                </button>
                {generating&&(
                  <div>
          ...
          {/* Right â€” video player + export (only when video exists) */}
          {videoUrl&&(
              ...
              <div style={{position:"relative",background:"#000"}}>
                ...
                <div style={{background:"rgba(0,0,0,0.85)",padding:"8px 12px"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <button onClick={()=>videoRef.current&&(videoRef.current.currentTime=0)} style={{background:"none",border:"none",color:GOLDDIM,cursor:"pointer",fontSize:14}}>â®</button>
                      ...
                      <button onClick={()=>videoRef.current&&(videoRef.current.currentTime=Math.min(duration2,videoRef.current.currentTime+10))} style={{background:"none",border:"none",color:GOLDDIM,cursor:"pointer",fontSize:14}}>â©</button>
              ...
              <div style={{flex:1,overflowY:"auto",padding:"12px 14px"}}>
                ...
                <button onClick={()=>{
                  if(videoBlob&&onSave){
                    ...
                    addLog("âœ“ Saved to media library");
                  }
        ...
        {!videoUrl&&(
          <div style={{borderTop:"1px solid "+GOLDDIM+"",padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
            <button onClick={()=>setStep(s=>Math.max(1,s-1))} disabled={step===1} style={{background:"transparent",border:"1px solid "+GOLD,color:GOLD,padding:"6px 16px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",opacity:step===1?0.3:1}}>â—€ BACK</button>
            <span style={{color:GOLDDIM,fontSize:10,letterSpacing:2}}>STEP {step} OF 4</span>
            {step<4
              ?<button onClick={()=>setStep(s=>Math.min(4,s+1))} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"6px 16px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>NEXT â–¶</button>
...
const VOICE_CHARACTERS = [
  {id:"amanda",name:"Amanda",emoji:"â­",gender:"Female",age:"Adult",origin:"Founder",region:"MandaStrong",style:"Your voice Â· Narrator",pitch:1.0,rate:0.85,desc:"Amanda's own voice â€” your recorded narration.",isOwner:true},
  {id:"james",name:"James",emoji:"ðŸŽ©",gender:"Male",age:"Adult",origin:"British",region:"London",style:"Sarcastic Â· Deadpan Â· Witty",pitch:0.86,rate:0.62,desc:"Dry British wit. Devastating things said with complete calm."},
  {id:"aurora",name:"Aurora",emoji:"ðŸŒ…",gender:"Female",age:"Adult",origin:"British",region:"London",style:"Warm Â· Documentary Â· Authoritative",pitch:1.08,rate:0.80,desc:"Calm authority. The voice you trust completely."},
  {id:"edward",name:"Edward",emoji:"ðŸŽ­",gender:"Male",age:"Adult",origin:"British",region:"London",style:"Theatrical Â· Grand Â· Classical",pitch:0.85,rate:0.75,desc:"Shakespearean gravitas. Every sentence carved in stone."},
  {id:"cecily",name:"Cecily",emoji:"ðŸ«–",gender:"Female",age:"Adult",origin:"British",region:"London",style:"Crisp Â· Intelligent Â· Sardonic",pitch:1.12,rate:0.85,desc:"Sharp as a tack. Mildly disappointed by most things."},
  {id:"nana",name:"Nana",emoji:"ðŸ§¶",gender:"Female",age:"Elderly",origin:"British",region:"Yorkshire",style:"Gentle Â· Wise Â· Warm",pitch:1.02,rate:0.70,desc:"Warm elderly wisdom. Has seen everything twice."},
  {id:"colonel",name:"Colonel",emoji:"ðŸŽ–ï¸",gender:"Male",age:"Elderly",origin:"British",region:"London",style:"Commanding Â· Dignified Â· Veteran",pitch:0.80,rate:0.74,desc:"Authority earned through decades of experience."},
  {id:"pippa",name:"Pippa",emoji:"ðŸŽ€",gender:"Female",age:"Teen",origin:"British",region:"London",style:"Bright Â· Cheerful Â· Young",pitch:1.25,rate:0.95,desc:"Fresh and warm. Natural young British energy."},
  {id:"archie",name:"Archie",emoji:"âš½",gender:"Male",age:"Teen",origin:"British",region:"Manchester",style:"Casual Â· Friendly Â· Teen",pitch:1.05,rate:0.98,desc:"Relaxed and genuine. Sounds like a real teenager."},
  {id:"ewan",name:"Ewan",emoji:"ðŸ´",gender:"Male",age:"Adult",origin:"Scottish",region:"Edinburgh",style:"Warm Â· Rugged Â· Sincere",pitch:0.92,rate:0.82,desc:"Deep warm Scottish sincerity."},
  {id:"fiona",name:"Fiona",emoji:"ðŸŒ¿",gender:"Female",age:"Adult",origin:"Scottish",region:"Glasgow",style:"Lilting Â· Warm Â· Storyteller",pitch:1.10,rate:0.84,desc:"Beautiful Scottish lilt."},
  {id:"paddy",name:"Paddy",emoji:"â˜˜ï¸",gender:"Male",age:"Adult",origin:"Irish",region:"Dublin",style:"Charming Â· Witty Â· Warm",pitch:0.95,rate:0.88,desc:"Easy Irish charm."},
  {id:"siobhan",name:"Siobhan",emoji:"ðŸŒ¸",gender:"Female",age:"Adult",origin:"Irish",region:"Cork",style:"Gentle Â· Musical Â· Emotional",pitch:1.15,rate:0.82,desc:"Soft Irish voice with real emotional depth."},
  {id:"dafydd",name:"Dafydd",emoji:"ðŸ‰",gender:"Male",age:"Adult",origin:"Welsh",region:"Cardiff",style:"Musical Â· Passionate Â· Rich",pitch:0.90,rate:0.80,desc:"Rich Welsh musicality."},
  {id:"marcus",name:"Marcus",emoji:"âš¡",gender:"Male",age:"Adult",origin:"American",region:"New York",style:"Deep Â· Cinematic Â· Commanding",pitch:0.72,rate:0.74,desc:"Big voice. When Marcus speaks people stop."},
  {id:"river",name:"River",emoji:"ðŸŒŠ",gender:"Male",age:"Adult",origin:"American",region:"Tennessee",style:"Warm Â· Intimate Â· Storyteller",pitch:0.98,rate:0.76,desc:"Unhurried Southern charm."},
  {id:"dakota",name:"Dakota",emoji:"ðŸ”ï¸",gender:"Female",age:"Adult",origin:"American",region:"Chicago",style:"Bold Â· Direct Â· Confident",pitch:1.05,rate:0.92,desc:"No filler. No hesitation."},
  {id:"wade",name:"Wade",emoji:"ðŸ¤ ",gender:"Male",age:"Adult",origin:"American",region:"Texas",style:"Laid Back Â· Humorous Â· Folksy",pitch:0.94,rate:0.85,desc:"Easy going Southern humour."},
  {id:"brooklyn",name:"Brooklyn",emoji:"ðŸ—½",gender:"Female",age:"Adult",origin:"American",region:"New York",style:"Fast Â· Sharp Â· City Energy",pitch:1.18,rate:1.10,desc:"Fast New York energy."},
  {id:"savannah",name:"Savannah",emoji:"ðŸŒº",gender:"Female",age:"Adult",origin:"American",region:"Georgia",style:"Sweet Â· Gracious Â· Warm",pitch:1.20,rate:0.84,desc:"Warm Southern grace."},
  {id:"madison",name:"Madison",emoji:"ðŸ“±",gender:"Female",age:"Teen",origin:"American",region:"California",style:"Upbeat Â· Social Â· Natural",pitch:1.30,rate:1.08,desc:"Real American teenage energy."},
  {id:"tyler",name:"Tyler",emoji:"ðŸŽ®",gender:"Male",age:"Teen",origin:"American",region:"Ohio",style:"Casual Â· Relatable Â· Teen",pitch:1.08,rate:1.00,desc:"Natural and unforced."},
  {id:"rosie",name:"Rosie",emoji:"ðŸŒ¼",gender:"Female",age:"Child",origin:"American",region:"Florida",style:"Sweet Â· Innocent Â· Child",pitch:1.45,rate:0.88,desc:"Young warm and sweet."},
  {id:"cooper",name:"Cooper",emoji:"ðŸš‚",gender:"Male",age:"Child",origin:"American",region:"Colorado",style:"Bright Â· Curious Â· Child",pitch:1.40,rate:0.90,desc:"Curious about everything."},
  {id:"grandma",name:"Grandma",emoji:"ðŸ«¶",gender:"Female",age:"Elderly",origin:"American",region:"Virginia",style:"Warm Â· Loving Â· Elderly",pitch:1.00,rate:0.72,desc:"Full of love and life experience."},
  {id:"frank",name:"Frank",emoji:"ðŸª‘",gender:"Male",age:"Elderly",origin:"American",region:"New Jersey",style:"Gruff Â· Honest Â· Elder",pitch:0.78,rate:0.76,desc:"Says it straight."},
  {id:"sophia",name:"Sophia",emoji:"â˜€ï¸",gender:"Female",age:"Adult",origin:"Australian",region:"Sydney",style:"Upbeat Â· Bright Â· Energetic",pitch:1.35,rate:1.12,desc:"Forward energy."},
  {id:"finn",name:"Finn",emoji:"ðŸ„",gender:"Male",age:"Adult",origin:"Australian",region:"Melbourne",style:"Casual Â· Confident Â· Outdoorsy",pitch:0.95,rate:0.95,desc:"Relaxed Australian confidence."},
  {id:"aroha",name:"Aroha",emoji:"ðŸŒ¿",gender:"Female",age:"Adult",origin:"New Zealand",region:"Auckland",style:"Warm Â· Grounded Â· Sincere",pitch:1.10,rate:0.86,desc:"Natural sincerity."},
  {id:"amara",name:"Amara",emoji:"ðŸŒ",gender:"Female",age:"Adult",origin:"South African",region:"Cape Town",style:"Rich Â· Warm Â· Powerful",pitch:1.05,rate:0.84,desc:"Quiet power."},
  {id:"kofi",name:"Kofi",emoji:"ðŸ¥",gender:"Male",age:"Adult",origin:"West African",region:"Ghana",style:"Deep Â· Rhythmic Â· Storyteller",pitch:0.82,rate:0.78,desc:"Every sentence has music in it."},
  {id:"priya",name:"Priya",emoji:"ðŸª·",gender:"Female",age:"Adult",origin:"Indian",region:"Mumbai",style:"Precise Â· Warm Â· Intelligent",pitch:1.15,rate:0.90,desc:"Warm and intelligent."},
  {id:"arjun",name:"Arjun",emoji:"ðŸŽ¯",gender:"Male",age:"Adult",origin:"Indian",region:"Delhi",style:"Authoritative Â· Clear Â· Measured",pitch:0.88,rate:0.85,desc:"Sounds like someone who knows exactly what they are talking about."},
  {id:"valentina",name:"Valentina",emoji:"ðŸŒ¹",gender:"Female",age:"Adult",origin:"Spanish",region:"Madrid",style:"Passionate Â· Warm Â· Expressive",pitch:1.18,rate:0.92,desc:"Everything sounds felt."},
  {id:"pierre",name:"Pierre",emoji:"ðŸ¥",gender:"Male",age:"Adult",origin:"French",region:"Paris",style:"Suave Â· Dry Â· Cultured",pitch:0.90,rate:0.84,desc:"Makes things sound interesting."},
  {id:"ingrid",name:"Ingrid",emoji:"â„ï¸",gender:"Female",age:"Adult",origin:"Scandinavian",region:"Stockholm",style:"Clean Â· Cool Â· Direct",pitch:1.08,rate:0.88,desc:"No excess words."},
  {id:"yemi",name:"Yemi",emoji:"ðŸŒŸ",gender:"Female",age:"Adult",origin:"Nigerian",region:"Lagos",style:"Bold Â· Joyful Â· Energetic",pitch:1.25,rate:1.00,desc:"Life-affirming."},
  {id:"magnus",name:"Magnus",emoji:"ðŸ§™",gender:"Male",age:"Elderly",origin:"Fantasy",region:"Ancient",style:"Ancient Â· Wise Â· Epic",pitch:0.75,rate:0.70,desc:"Seen civilisations rise and fall."},
  {id:"nova",name:"Nova",emoji:"ðŸ¤–",gender:"Female",age:"Adult",origin:"Neutral",region:"AI",style:"Clean Â· Precise Â· Neutral",pitch:1.12,rate:0.95,desc:"No accent. No emotion. No opinion."},
  {id:"hunter",name:"Hunter",emoji:"ðŸŽ¬",gender:"Male",age:"Adult",origin:"American",region:"Hollywood",style:"Trailer Â· Epic Â· Explosive",pitch:0.70,rate:0.80,desc:"Full movie trailer energy."},
  {id:"luna",name:"Luna",emoji:"ðŸŒ™",gender:"Female",age:"Adult",origin:"Neutral",region:"ASMR",style:"Whisper Â· ASMR Â· Intimate",pitch:1.20,rate:0.65,desc:"Soft whisper. Complete calm."},
  {id:"professor",name:"Professor",emoji:"ðŸŽ“",gender:"Male",age:"Elderly",origin:"British",region:"Oxford",style:"Academic Â· Thoughtful Â· Measured",pitch:0.88,rate:0.78,desc:"Distinguished. Precise."},
  {id:"hope",name:"Hope",emoji:"ðŸŒ¤ï¸",gender:"Female",age:"Adult",origin:"American",region:"Heartfelt",style:"Tender Â· Gentle Â· Loving",pitch:1.15,rate:0.78,desc:"Pure tenderness."},
  {id:"storm",name:"Storm",emoji:"â›ˆï¸",gender:"Male",age:"Adult",origin:"American",region:"Intense",style:"Intense Â· Angry Â· Powerful",pitch:0.82,rate:1.00,desc:"Raw intensity."},
  {id:"joy",name:"Joy",emoji:"ðŸŽ‰",gender:"Female",age:"Adult",origin:"American",region:"Uplifting",style:"Excited Â· Joyful Â· Celebratory",pitch:1.40,rate:1.15,desc:"Pure infectious joy."},
  {id:"sage",name:"Sage",emoji:"ðŸŒ¿",gender:"Male",age:"Adult",origin:"Neutral",region:"Mindful",style:"Peaceful Â· Mindful Â· Grounded",pitch:0.95,rate:0.72,desc:"Deep calm."},
  {id:"faith",name:"Faith",emoji:"âœ¨",gender:"Female",age:"Adult",origin:"American",region:"Gospel",style:"Inspirational Â· Gospel Â· Uplifting",pitch:1.18,rate:0.88,desc:"Gospel soul."},
  {id:"rebel",name:"Rebel",emoji:"âœŠ",gender:"Female",age:"Teen",origin:"American",region:"Activist",style:"Fierce Â· Defiant Â· Young",pitch:1.22,rate:1.05,desc:"Will not back down."},
  {id:"blaze",name:"Blaze",emoji:"ðŸ”¥",gender:"Female",age:"Adult",origin:"American",region:"Cinematic",style:"Warm Â· Confident Â· Cinematic",pitch:1.02,rate:0.95,desc:"Warm cinematic narrator."},
  {id:"remy",name:"Remy",emoji:"ðŸŽ»",gender:"Male",age:"Adult",origin:"French",region:"Lyon",style:"Smooth Â· Romantic Â· Intimate",pitch:0.92,rate:0.80,desc:"Everything sounds like poetry."},
  {id:"zhara",name:"Zhara",emoji:"ðŸ’«",gender:"Female",age:"Adult",origin:"Middle Eastern",region:"Dubai",style:"Elegant Â· Warm Â· Sophisticated",pitch:1.10,rate:0.85,desc:"Graceful and precise."},
  {id:"kai",name:"Kai",emoji:"ðŸŒŠ",gender:"Male",age:"Adult",origin:"Hawaiian",region:"Honolulu",style:"Relaxed Â· Warm Â· Soulful",pitch:0.96,rate:0.82,desc:"Unhurried ocean warmth."},
  {id:"sienna",name:"Sienna",emoji:"ðŸŽ¨",gender:"Female",age:"Adult",origin:"American",region:"New Orleans",style:"Soulful Â· Blues Â· Deep",pitch:1.05,rate:0.78,desc:"Every word feels lived-in."},
  {id:"atlas",name:"Atlas",emoji:"ðŸŒ",gender:"Male",age:"Adult",origin:"Neutral",region:"Epic",style:"Cinematic Â· Epic Â· Booming",pitch:0.68,rate:0.76,desc:"The voice of a thousand documentaries."},
  {id:"echo",name:"Echo",emoji:"ðŸ”®",gender:"Female",age:"Adult",origin:"Neutral",region:"Ethereal",style:"Ethereal Â· Dreamy Â· Otherworldly",pitch:1.22,rate:0.72,desc:"Sounds like it came from somewhere else."},
];
...
function P6Voice({ onSave, setMediaLib }) {
  ...
  // died on refresh â€” that is why a recorded voice never actually played back.)
  const addMyVoice=async(file)=>{
    if(!file)return;
    const id="myvoice_"+Date.now();
    const url=URL.createObjectURL(file);
    const nv={id,name:(file.name||"My Voice").replace(/\.[^.]+$/,""),url,dbId:id,origin:"My Upload",gender:"â€”",age:"â€”",emoji:"ðŸŽ™",style:"Your recorded voice",desc:"Your own voice â€” plays back exactly as recorded."};
  ...
  // Save the SELECTED own-voice recording as the film's narration â€” real audio,
  ...
  const saveMyVoiceAsNarration=async()=>{
    ...
    if(!blob){alert("Could not find that recording's audio â€” try recording again.");return;}
  ...
  // â”€â”€ HIDDEN CLONE FEATURE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const cloneMyVoice=async()=>{
    ...
    try{
      ...
      try{const st=await loadClipFromDB(mine.dbId||mine.id);if(st&&st.blob)blob=st.blob;}catch(e){}
      if(!blob&&mine.url){try{blob=await (await fetch(mine.url)).blob();}catch(e){}}
      if(!blob){setCloning(false);alert("Could not find that recording's audio â€” try recording again.");return;}
      ...
      const upd=myVoices.map(v=>v.id===mine.id?{...v,clonedVoiceId:vid,engineVoice:vid,desc:"Your cloned voice â€” the engine narrates new text in your voice.",style:"Your cloned voice"}:v);
      setMyVoices(upd);
      try{localStorage.setItem("ms_my_voices",JSON.stringify(upd.map(v=>({...v,url:undefined}))));}catch{}
      setCloning(false);
      alert("Cloned. Select this voice and the engine will narrate any text in your voice.");
    }catch(e){setCloning(false);alert("Voice clone failed â€” try again.");}
  };

  // â”€â”€ CONSENT + COMPLETE-NARRATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const engineCompleteNarration=async()=>{
    ...
    try{
      ...
      if(!vid){
        ...
        try{const st=await loadClipFromDB(mine.dbId||mine.id);if(st&&st.blob)blob=st.blob;}catch(e){}
        if(!blob&&mine.url){try{blob=await (await fetch(mine.url)).blob();}catch(e){}}
        if(!blob){setNarrBusy(false);alert("Could not find that recording's audio â€” try recording again.");return;}
        const dataUri=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result);r.onerror=rej;r.readAsDataURL(blob);});
      ...
      setNarrBusy(false);
      setSavedToLib(true);setTimeout(()=>setSavedToLib(false),3000);
      alert("Done â€” the engine narrated your full script in your voice and saved it to the timeline for the render.");
    }catch(e){setNarrBusy(false);alert("Could not complete the narration â€” try again.");}
  };
  ...
  const pickSysVoice=(vc)=>{
    ...
    // â”€â”€ QUALITY FIRST â€” use Enhanced/Premium/Siri/Neural voices when present â”€â”€
  ...
  const speakDevice=(txt)=>{
    ...
    // iOS Safari fix â€” keepalive ping every 10s
  ...
  const speakNow=async(txt)=>{
    ...
    if(!probe){ console.log("Cinema Voice Engine unavailable â€” using device voice"); speakDevice(txt); return; }
    ...
    for(let i=0;i<chunks.length;i++){
      ...
      if(!ok){ console.log("Cinema Voice Engine playback blocked â€” using device voice"); speakDevice(txt); return; }
  ...
  return(
    <div style={{...Sp}}>
      ...
      <div style={{padding:"12px 18px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div><div style={{fontSize:11,color:GOLD,letterSpacing:4,fontWeight:700}}>AI WORKSTATION 02 â€” CINEMA VOICE ENGINE</div><h1 style={{...H1,fontSize:24,margin:0}}>TEXT TO LIFELIKE SPEECH</h1></div>
        <button onClick={()=>setShowMVS(true)} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"10px 20px",cursor:"pointer",fontSize:12,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>ðŸŽ¬ MUSIC VIDEO STUDIO</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"290px 1fr",minHeight:"calc(100vh - 120px)"}}>
        <div style={{borderRight:"1px solid "+GOLDDIM+"",background:"#030303",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"10px 10px 6px"}}>
            <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900,marginBottom:8}}>VOICE LIBRARY â€” {filtered.length} / {VOICE_CHARACTERS.length}</div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"6px 6px 80px"}}>
            ...
            {recordingMine?(
              <div style={{display:"flex",alignItems:"center",gap:8,background:"#1a0000",border:"2px solid #ef4444",padding:"9px 12px",marginBottom:6}}>
                <div style={{width:10,height:10,borderRadius:"50%",background:"#ef4444",boxShadow:"0 0 8px #ef4444"}}/>
                <span style={{color:"#ef4444",fontWeight:900,fontSize:11,letterSpacing:2,flex:1}}>RECORDING â€” {String(Math.floor(recTime/60)).padStart(2,"0")}:{String(recTime%60).padStart(2,"0")}</span>
                <button onClick={stopMyRecording} style={{background:"#ef4444",border:"none",color:"#fff",padding:"5px 14px",cursor:"pointer",fontSize:10,fontWeight:900,letterSpacing:2}}>â–  STOP & SAVE</button>
              </div>
            ):(
              <button onClick={startMyRecording} style={{width:"100%",background:"linear-gradient(135deg,#7a0000,#ef4444)",border:"none",color:"#fff",padding:"10px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",marginBottom:6}}>â— RECORD MY VOICE NOW</button>
            )}
            <button onClick={()=>myVoiceInputRef.current&&myVoiceInputRef.current.click()} style={{width:"100%",background:"linear-gradient(135deg,#1a0800,#2a1200)",border:"2px solid "+GOLD,color:GOLD,padding:"10px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",marginBottom:6}}>ï¼‹ ADD YOUR OWN VOICE (FILE)</button>
            {myVoices.some(v=>v.id===selVoice)&&(<>
              <div style={{color:GOLDDIM,fontSize:10,lineHeight:1.5,marginBottom:6,letterSpacing:0.5}}>Record just the FIRST PARAGRAPH in your own voice â€” the engine clones your voice and reads the rest of the narration to the end in YOUR voice, wired into the generator and render.</div>
              ...
              {narrConsent==="yes"&&(<>
                <button onClick={saveMyVoiceAsNarration} style={{width:"100%",background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"11px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",marginBottom:6}}>ðŸŽ™ USE MY VOICE AS NARRATION</button>
                <button onClick={engineCompleteNarration} disabled={narrBusy} style={{width:"100%",background:narrBusy?"#1a0800":"linear-gradient(135deg,#1a0800,#2a1200)",border:"2px solid "+GOLD,color:GOLD,padding:"11px",cursor:narrBusy?"wait":"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",marginBottom:6}}>{narrBusy?"âŸ³ CLONING & COMPLETINGâ€¦":"ðŸŽ§ USE ENGINE TO COMPLETE FULL NARRATION"}</button>
              </>)}
            </>)}
            {myVoices.map(v=>(
              <div key={v.id} onClick={()=>setSelVoice(v.id)} style={{padding:"10px 12px",marginBottom:4,background:selVoice===v.id?"#0a0800":"#000",border:"2px solid "+(selVoice===v.id?GOLD:GOLDDIM),cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <div><div style={{color:selVoice===v.id?GOLD:WHITE,fontSize:13,fontWeight:900}}>{v.name}{v.clonedVoiceId&&<span style={{color:GOLD,fontSize:10,marginLeft:6}}>âœ¦ CLONED</span>}</div><div style={{color:GOLDDIM,fontSize:10}}>{v.clonedVoiceId?"Your cloned voice â€” engine narrates in your voice":"Your uploaded voice"}</div></div>
                  </div>
                  <div style={{display:"flex",gap:4,flexShrink:0}}>
                    {v.url&&<button onClick={e=>{e.stopPropagation();const a=new Audio(v.url);a.play().catch(()=>{});}} style={{background:GOLDDIM,border:"none",color:"#000",padding:"3px 8px",cursor:"pointer",fontSize:9,fontWeight:900}}>â–¶</button>}
                    <button onClick={e=>{e.stopPropagation();delMyVoice(v.id);}} style={{background:"#000",border:"1px solid "+GOLD,color:GOLD,padding:"3px 8px",cursor:"pointer",fontSize:9,fontWeight:900}}>âœ•</button>
                  </div>
                </div>
                {selVoice===v.id&&<div style={{color:GOLD,fontSize:9,letterSpacing:2,marginTop:4,fontWeight:900}}>{cloning?"âœ¦ CLONING YOUR VOICEâ€¦":"âœ“ SELECTED"}</div>}
              </div>
            ))}
            {filtered.map(v=>(
              <div key={v.id} onClick={()=>setSelVoice(v.id)} style={{padding:"10px 12px",marginBottom:4,background:selVoice===v.id?"#0a0800":"#000",border:"2px solid "+(selVoice===v.id?GOLD:GOLDDIM),cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <div><div style={{color:selVoice===v.id?GOLD:WHITE,fontSize:13,fontWeight:900}}>{v.name}</div><div style={{color:GOLDDIM,fontSize:10}}>{v.origin} Â· {v.gender} Â· {v.age}</div></div>
                  </div>
                  <button onClick={e=>{e.stopPropagation();setSelVoice(v.id);speakOneShot(v,"Hello, this is "+v.name+". "+v.desc);}}
                    style={{background:GOLDDIM,border:"none",color:"#000",padding:"3px 10px",cursor:"pointer",fontSize:9,fontWeight:900,letterSpacing:1,fontFamily:"'Rajdhani',sans-serif",whiteSpace:"nowrap",flexShrink:0}}>â–¶ TEST</button>
                </div>
                <div style={{color:DIM,fontSize:10,lineHeight:1.5}}>{v.style}</div>
                {selVoice===v.id&&<div style={{color:GOLD,fontSize:9,letterSpacing:2,marginTop:4,fontWeight:900}}>âœ“ SELECTED</div>}
              </div>
            ))}
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",background:"#030303",overflowY:"auto",padding:20}}>
          <div style={{background:"#000",border:"1px solid "+GOLDDIM,padding:"10px 14px",marginBottom:14}}>
            <div style={{color:WHITE,fontSize:13,fontWeight:900}}>{selected.name} {selected.emoji} Â· {selected.origin} Â· {selected.gender}</div>
          ...
          <button onClick={()=>{
            ...
            {speaking?"â¹ STOP":"âœ¦ PREPARE TO SPEAK"}
          </button>
          ...
          {savedToLib&&<div style={{background:"#061406",border:"1px solid #22c55e",padding:"10px 14px",textAlign:"center",marginBottom:8}}><span style={{color:"#22c55e",fontWeight:900,fontSize:12,letterSpacing:2}}>âœ“ NARRATION SAVED TO MEDIA LIBRARY â€” AUTO-ADDED TO TIMELINE</span></div>}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{display:"flex",justifyContent:"center",marginBottom:24}}>
      <div style={{background:"#050500",border:"2px solid "+GOLD,padding:"14px 40px",textAlign:"center",boxShadow:"0 0 24px "+GOLD+"33",position:"relative"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:6}}>
          ...
          <div style={{color:"#22c55e",fontSize:10,letterSpacing:4,fontWeight:900}}>LIVE Â· USER COUNT</div>
        </div>
        <div style={{display:"flex",alignItems:"flex-end",justifyContent:"center",gap:34}}>
          <div>
            ...
            <div style={{color:"#22c55e",fontSize:9,letterSpacing:3,marginTop:4}}>â— ON NOW</div>
          </div>
        ...
        <div style={{color:GOLDDIM,fontSize:9,letterSpacing:3,marginTop:8}}>launched june 1st 2026 Â· updates automatically</div>
...
function P8VideoGenerator({ onSave, user, filmDuration, setFilmDuration }) {
  ...
  const [videoUrl,setVideoUrl]=useState("");
  const [saved,setSaved]=useState(false);
  // â”€â”€ BACKGROUND MUSIC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const MUSIC_LIBRARY=[
    {id:"epic",     label:"âš”ï¸ Epic / Trailer",       url:"https://cdn.pixabay.com/audio/2022/03/15/audio_c8c8a73467.mp3"},
    {id:"drama",    label:"ðŸŽ­ Drama / Powerful",     url:"https://cdn.pixabay.com/audio/2023/01/29/audio_5bf2f9f5b0.mp3"},
    {id:"emotional",label:"ðŸ’§ Emotional / Piano",    url:"https://cdn.pixabay.com/audio/2021/11/25/audio_00fa5593f3.mp3"},
    {id:"ambient",  label:"ðŸŒŒ Ambient / Cinematic",  url:"https://cdn.pixabay.com/audio/2022/01/18/audio_d0c6ff1bab.mp3"},
    {id:"uplifting",label:"â˜€ï¸ Uplifting / Hopeful",   url:"https://cdn.pixabay.com/audio/2022/05/27/audio_1808fbf07a.mp3"},
    {id:"tension",  label:"ðŸŽ¯ Tension / Suspense",    url:"https://cdn.pixabay.com/audio/2022/03/10/audio_d1718ab41b.mp3"},
    {id:"warm",     label:"ðŸ•¯ï¸ Warm / Reflective",     url:"https://cdn.pixabay.com/audio/2021/08/09/audio_54ca0ffa52.mp3"},
    {id:"inspiring",label:"ðŸŒ… Inspiring / Anthemic",  url:"https://cdn.pixabay.com/audio/2022/10/25/audio_946bc8a627.mp3"},
    {id:"sad",      label:"ðŸŒ§ï¸ Sad / Melancholy",      url:"https://cdn.pixabay.com/audio/2022/10/18/audio_31a1f6f2a6.mp3"},
    {id:"action",   label:"ðŸ’¥ Action / Driving",      url:"https://cdn.pixabay.com/audio/2022/11/22/audio_febc508520.mp3"},
  ];
  ...
  const RENDER_STYLES=[
    {id:"photorealistic",label:"ðŸ“· Photorealistic"},
    {id:"cinematic",label:"ðŸŽ¬ Cinematic"},
    {id:"documentary",label:"ðŸŽ¥ Documentary"},
    {id:"noir",label:"ðŸŒ‘ Film Noir"},
    {id:"golden",label:"ðŸŒ… Golden Hour"},
    {id:"scifi",label:"ðŸš€ Sci-Fi"},
    {id:"horror",label:"ðŸ‘» Horror"},
    {id:"animated",label:"âœ¨ Stylised"},
  ];
  const FILM_GENRES=[
    {id:"",label:"â€” NO GENRE â€”"},
    {id:"feature",label:"ðŸŽ¬ Feature Film"},{id:"documentary",label:"ðŸŽ¥ Documentary"},
    {id:"musicvideo",label:"ðŸŽµ Music Video"},{id:"shortfilm",label:"ðŸŽ­ Short Film"},
    {id:"horror",label:"ðŸ‘» Horror"},{id:"scifi",label:"ðŸš€ Sci-Fi"},
    {id:"romance",label:"ðŸ’• Romance"},{id:"thriller",label:"âš¡ Thriller"},
    {id:"action",label:"ðŸ’¥ Action"},{id:"comedy",label:"ðŸ˜„ Comedy"},
    {id:"drama",label:"ðŸŽ­ Drama"},{id:"animation",label:"âœ¨ Animation"},
    {id:"historical",label:"ðŸ› Historical"},{id:"nature",label:"ðŸŒ¿ Nature"},
  ];
  const EXAMPLES=[
  ...
  // MANDASTRONG ENGINE v2 â€” CINEMA-GRADE RENDERER
  // Real depth, real volumetric lighting, real atmosphere, real motion
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  // MANDASTRONG ENGINE â€” AI writes a custom drawFrame() per prompt
  ...
  const generateVideo=async()=>{
    ...
    // â”€â”€ PRIORITY SAVE â€” runs before anything else â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try{
      ...
      // These are already up to date from auto-persist â€” just verify they're written
    ...
    // â”€â”€ PRE-GENERATION STORAGE CHECK â€” safe, never touches source clips â”€â”€
    ...
    addLog("MandaStrong Cinema Engine â€” reading your scene...");
    setProgress(5);

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // MANDASTRONG CINEMA ENGINE â€” REAL PHOTOREALISTIC VIDEO
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    try{
      addLog("Cinema Engine â€” synthesising photorealistic footage...");
      setProgress(12);
      // â”€â”€ SCRIPT-TO-MOVIE BRIEF â”€â”€ Page 5's Producer/Describe/Production
      ...
      if(useBrief){
        try{
          ...
          if(bd&&bd.brief){ effectivePrompt=bd.brief+"\nSHOT FOR THIS SCENE:\n"+effectivePrompt; addLog("âœ¦ Using Script-to-Movie brief (Producer + Describe + Production) from Page 5"); }
        }catch(e){}
      }
      const engineUrl=await engineRender(effectivePrompt,{
        duration,
        image:refDataUrl||"",
        onTick:(i)=>{ setProgress(Math.min(88,12+i*2)); addLog("Cinema Engine rendering â€” "+Math.min(88,12+i*2)+"%"); }
      });
      if(engineUrl){
        addLog("\u2713 Cinema Engine complete â€” downloading footage...");
        setProgress(92);
        ...
        setProgress(100);
        addLog("\u2713 MANDASTRONG CINEMA ENGINE â€” photorealistic scene ready");
        try{
      ...
      const diag=await engineStatus();
      addLog(diag&&diag.ok===false?("Cinema Engine: "+(diag.message||"unavailable")):"Cinema Engine returned no footage â€” using built-in renderer");
    }catch(e){
      addLog("Cinema Engine offline â€” using built-in renderer");
    }
    ...
    if(refImages.length>0){
      ...
      try{
        ...
        addLog("\u2713 "+loadedRefImages.length+" photo(s) loaded â€” photorealistic mode");
    ...
    // â”€â”€ STEP 1: Ask Claude to write a custom drawFrame for this exact scene â”€â”€
    let drawFnBody="";
    try{
      addLog("MandaStrong Engine â€” asking AI to compose your scene...");
      setProgress(12);
      const hasPhotos=loadedRefImages.length>0;
      const photoNote=hasPhotos?"The user has uploaded "+loadedRefImages.length+" reference photo(s). The main photo will be drawn as the base layer already â€” your drawFrame should add atmosphere, lighting, overlays, and cinematic elements ON TOP of the photo base. Do NOT try to redraw the background from scratch.":"No reference photos. You must paint the entire scene from scratch using canvas drawing primitives â€” sky, ground, environment, people, objects, lighting. Make it look as photorealistic as possible using gradients, layering, and detail.";
...
Write a JavaScript function body (NOT the function declaration â€” just the code inside the braces) for:
...
2. Animate with t and sec â€” camera drift, parallax, light shifts, subtle motion.
...
6. No external images, no fetch calls, no DOM access â€” only ctx drawing.
...
Write the drawFrame body now.`}]
      ...
      drawFnBody=code;
      addLog("\u2713 Scene composed â€” "+drawFnBody.split("\n").length+" render instructions");
      setProgress(28);
    }catch(e){
      addLog("Scene compose error: "+e.message+" â€” using fallback renderer");
    ...
    // â”€â”€ STEP 2: Set up canvas + MediaRecorder â”€â”€
    const canvas=canvasRef.current;
    if(!canvas){setGenerating(false);addLog("Canvas error");return;}
    canvas.width=1280;canvas.height=720;
    const ctx=canvas.getContext("2d");

    // Build the drawFrame function â€” wraps AI code + photo base layer
    let drawFrame;
    try{
      if(loadedRefImages.length>0){
        ...
        drawFrame=new Function("ctx","W","H","t","sec","loadedRefImages",`
          // Photo base â€” Ken Burns push-in
    ...
    // â”€â”€ STEP 3: Render all frames â”€â”€
    const fps=20;const totalFrames=duration*fps;
    const mimeType=MediaRecorder.isTypeSupported("video/webm;codecs=vp9")?"video/webm;codecs=vp9":"video/webm";
    const stream=canvas.captureStream(fps);
    // â”€â”€ BACKGROUND MUSIC â€” mix chosen track under the render â”€â”€
      ...
      try{
        ...
        if(track){
          ...
          if(genStereo && musicCtx.createStereoPanner){
            addLog("ðŸ”Š Stereo sound ON â€” widening the music bed");
          ...
          addLog("âœ“ Background music ready â€” mixing into film");
        }
      }catch(e){ addLog("Music note: "+e.message+" â€” rendering without music"); musicCtx=null; musicSource=null; }
    }
    ...
    recorder.ondataavailable=e=>{if(e.data.size>0)chunks.push(e.data);};
    recorder.start(Math.round(1000/fps));
    addLog("Rolling â€” rendering "+duration+"s at 24fps...");
    setProgress(35);
    ...
    await new Promise(resolve=>{
      ...
      const tick=()=>{
        ...
        // â”€â”€ AUTO-ENHANCEMENT â€” warm gold grade + contrast + highlight recovery â”€â”€
    ...
    await new Promise(resolve=>{
      ...
      // Arm the escape hatch FIRST â€” guaranteed exit even if stop() throws
    ...
    setVideoUrl(url);
    setProgress(100);
    addLog("\u2713 MANDASTRONG ENGINE COMPLETE â€” "+duration+"s cinema-grade video ready");
    // AUTO-SAVE the finished clip â€” timeout-protected so it can never stall the render
    try{
      const autoId="scene_"+Date.now();
      ...
      if(saveResult==="timeout"){addLog("Save is running in background â€” clip is ready above");}
      else{
  ...
  return (
    <div style={{minHeight:"100vh",background:"#000",color:WHITE,fontFamily:"'Rajdhani',sans-serif",paddingBottom:160}}>
      {gapPrompt&&(
        <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,0.94)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{width:"min(460px,95vw)",background:"#050505",border:"2px solid "+GOLD,padding:"24px 22px"}}>
            <div style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:16,fontWeight:900,letterSpacing:3,marginBottom:14,textAlign:"center"}}>â³ FILL THE GAP?</div>
      ...
      <div style={{padding:"12px 20px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:11,color:GOLD,letterSpacing:4,fontWeight:700}}>MANDASTRONG ENGINE v2 Â· CINEMA-GRADE RENDERER</div>
        ...
        <div style={{color:GOLD,fontSize:11,fontWeight:700,letterSpacing:2}}>âœ¦ MANDASTRONG ENGINE Â· ANY PROMPT Â· ANY SUBJECT</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 420px",minHeight:"calc(100vh - 120px)"}}>
        <div style={{padding:20,overflowY:"auto"}}>
          ...
          <div style={{background:"linear-gradient(135deg,#0a0500,#1a0a00)",border:"2px solid "+GOLD,padding:14,marginBottom:12,boxShadow:"0 0 20px "+GOLD+"22"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div>
                <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900}}>âœ¦ REALITY ENGINE â€” UPLOAD YOUR PHOTOS</div>
                <div style={{color:DIM,fontSize:10,marginTop:2}}>Upload 1-6 real photos or videos. Drag & drop or click. Guaranteed photorealistic â€” no cartoons.</div>
              </div>
            </div>
            {refImages.length>0&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:4,marginBottom:8}}>
                {refImages.map((ri,i)=>(
                  <div key={i} style={{position:"relative"}}>
                    ...
                    <button onClick={()=>setRefImages(p=>p.filter((_,j)=>j!==i))} style={{position:"absolute",top:1,right:1,background:"#000",border:"1px solid "+GOLD,color:GOLD,padding:"0 4px",cursor:"pointer",fontSize:9,fontWeight:900,lineHeight:1.2}}>âœ•</button>
            ...
            <div
              ...
              <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:2,pointerEvents:"none"}}>â¬† DRAG & DROP PHOTOS OR VIDEOS HERE</div>
              <div style={{color:GOLDDIM,fontSize:9,marginTop:3,pointerEvents:"none"}}>JPG Â· PNG Â· MP4 Â· MOV Â· up to 6 files Â· drop zone lights up gold when ready</div>
            </div>
            ...
            <button onClick={()=>{if(refImages.length>=6){alert("Max 6 photos");return;}realityPhotoRef.current&&realityPhotoRef.current.click();}} style={{width:"100%",background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"10px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>
              ðŸ“· {refImages.length===0?"ADD PHOTOS / VIDEOS (UP TO 6)":"ADD MORE â€” "+refImages.length+"/6 LOADED"}
            </button>
            <a href="https://photos.google.com" target="_blank" rel="noopener noreferrer"
              ...
              ðŸŒ CHROMEBOOK USERS â†’ OPEN GOOGLE PHOTOS â†’ download photo â†’ then Add Photos above
            </a>
            <div style={{color:GOLDDIM,fontSize:9,marginTop:5,letterSpacing:1,textAlign:"center"}}>1st photo = BACKGROUND Â· others = foreground layers Â· guarantees photorealistic output</div>
          </div>
          <div style={{background:"#0a0a0a",border:"1px solid "+GOLDDIM,padding:12,marginBottom:12}}>
            <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900,marginBottom:5}}>â¬† UPLOAD REFERENCE IMAGE (OPTIONAL)</div>
            {refMedia?(
              <div style={{position:"relative"}}>
                <img src={refMedia} alt="ref" style={{width:"100%",height:72,objectFit:"cover",border:"1px solid "+GOLD}}/>
                <button onClick={()=>{setRefMedia(null);setRefDataUrl(null);}} style={{position:"absolute",top:3,right:3,background:"#000",border:"1px solid "+GOLD,color:GOLD,padding:"1px 6px",cursor:"pointer",fontSize:10,fontWeight:900}}>âœ•</button>
                <div style={{color:"#22c55e",fontSize:9,fontWeight:900,letterSpacing:2,marginTop:3}}>âœ“ REFERENCE LOADED</div>
              </div>
            ):(
              <div
                ...
                <div style={{color:GOLD,fontSize:12,fontWeight:900,letterSpacing:2}}>â¬† DRAG & DROP or CLICK</div>
                <div style={{color:GOLDDIM,fontSize:10,marginTop:3,letterSpacing:1}}>JPG Â· PNG Â· MP4</div>
              </div>
            )}
          ...
          <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g. AI For Humanity â€” Chapter 1"
          ...
          <div style={{marginBottom:14}}>
            <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900,marginBottom:8}}>QUICK EXAMPLES â€” CLICK TO TRY</div>
          ...
          <div style={{background:"#0a0a0a",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
            ...
            {targetMin>0&&(
              <div style={{color:GOLDDIM,fontSize:10,marginTop:6,letterSpacing:1}}>
                Made so far: {fmtMin(madeSeconds)} of {targetMin}m Â· {gapSeconds>0?("gap "+fmtMin(gapSeconds)):"target reached âœ“"}
              </div>
            )}
          </div>
          <div style={{background:"#0a0a0a",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
          ...
          {/* â”€â”€ ADD BACKGROUND MUSIC? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          <div style={{background:"#0a0a0a",border:"1px solid "+GOLDDIM,padding:14,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{marginTop:12}}>
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <select value={musicTrack} onChange={e=>setMusicTrack(e.target.value)}
                    ...
                    <option value="">â€” Select background music â€”</option>
                  ...
                  <button onClick={()=>{const m=MUSIC_LIBRARY.find(x=>x.id===musicTrack);if(!m)return;try{const a=new Audio(m.url);a.volume=0.5;a.play().catch(()=>{});setTimeout(()=>{try{a.pause();}catch(e){}},6000);}catch(e){}}}
                    ...
                    style={{background:musicTrack?"none":"#111",border:"1px solid "+GOLDDIM,color:musicTrack?GOLD:"#555",padding:"9px 14px",cursor:musicTrack?"pointer":"not-allowed",fontSize:12,fontWeight:900}}>â–¶ PREVIEW</button>
                </div>
          ...
          {/* â”€â”€ USE STEREO SOUND â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          <div onClick={()=>setGenStereo(s=>!s)} style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"12px 14px",background:"#0a0a0a",border:"1px solid "+(genStereo?GOLD:GOLDDIM),cursor:"pointer"}}>
            <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(genStereo?GOLD:GOLDDIM),background:genStereo?GOLD:"transparent",color:"#000",fontWeight:900,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{genStereo?"âœ“":""}</div>
            <div><div style={{color:genStereo?GOLD:WHITE,fontWeight:900,fontSize:12,letterSpacing:1}}>ðŸ”Š USE STEREO SOUND</div><div style={{color:GOLDDIM,fontSize:10,marginTop:1}}>Full stereo width baked into the generated video's audio</div></div>
          </div>
          {/* â”€â”€ SCRIPT-TO-MOVIE BRIEF (from Page 5) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          {hasBrief&&(
            <div onClick={()=>setUseBrief(b=>!b)} style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"12px 14px",background:"#0a0a0a",border:"1px solid "+(useBrief?GOLD:GOLDDIM),cursor:"pointer"}}>
              <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(useBrief?GOLD:GOLDDIM),background:useBrief?GOLD:"transparent",color:"#000",fontWeight:900,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{useBrief?"âœ“":""}</div>
              <div><div style={{color:useBrief?GOLD:WHITE,fontWeight:900,fontSize:12,letterSpacing:1}}>ðŸŽ¬ USE SCRIPT-TO-MOVIE BRIEF</div><div style={{color:GOLDDIM,fontSize:10,marginTop:1}}>Your Producer, Describe &amp; Production notes from Page 5 drive this render</div></div>
            </div>
          )}
          <button onClick={generateVideo} disabled={generating||!prompt.trim()}
            ...
            {generating?"âŸ³ MANDASTRONG ENGINE RENDERING... "+progress+"%":"ðŸŽ¬ GENERATE SCENE"}
          </button>
        </div>
            ...
            <div style={{padding:"10px 14px",borderBottom:"1px solid "+GOLDDIM+"",display:"flex",flexDirection:"column",gap:6}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                <a href={videoUrl} download={(title||"scene")+"_"+duration+"s.webm"} target="_blank" rel="noopener noreferrer"
                  style={{background:"transparent",border:"1px solid "+GOLD,color:GOLD,padding:"8px",fontSize:10,textDecoration:"none",textAlign:"center",letterSpacing:1,fontWeight:900,fontFamily:"'Rajdhani',sans-serif",display:"block"}}>â¬‡ DOWNLOAD</a>
          ...
          <div style={{flex:1,overflowY:"auto",padding:14}}>
            {log.length>0?(
              ...
              <div style={{padding:"16px 0",color:GOLDDIM,fontSize:10,lineHeight:2.2,letterSpacing:1}}>
                ...
                âœ¦ 8 rendering layers per frame<br/>
                âœ¦ Multi-layer parallax depth<br/>
                âœ¦ Volumetric candle flickering<br/>
                âœ¦ Animated ocean (12 wave layers)<br/>
                âœ¦ Procedural human anatomy<br/>
                âœ¦ Real moon halos + reflections<br/>
                âœ¦ Three-tier city buildings<br/>
                âœ¦ Film grain + vignette + grade<br/>
                âœ¦ Camera push-in + drift<br/>
                âœ¦ 24fps Â· 1080p Â· 18Mbps
              </div>
            )}
          </div>
        </div>
    ...
    <div style={{...Sp}}>
      <div style={{background:"#000",padding:"56px 40px 36px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{fontSize:11,color:DIM,letterSpacing:6,marginBottom:12}}>CINEMA INTELLIGENCE PLATFORM â€” EST. 2025</div>
          ...
          <div style={{color:WHITE,fontSize:12,letterSpacing:4,marginBottom:28,fontWeight:600}}>600+ AI TOOLS Â· 8K EXPORT Â· UP TO 3-HOUR FILMS</div>
      ...
      <div style={{textAlign:"center",paddingBottom:24,paddingTop:16}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
          <button onClick={async()=>{
            ...
            if(window.deferredInstallPrompt){
              try{
                ...
                if(choice&&choice.outcome==="accepted") alert("âœ“ Installing MandaStrong Studio to your home screen. Look for the gold M icon.");
              }catch(e){}
            } else if(isIOS){
              alert("âœ“ MandaStrong Studio.html downloaded to your device.\n\nTo also add it to your home screen on iPhone/iPad:\n1. Tap the Share button â¬† in Safari\n2. Scroll down and tap 'Add to Home Screen'\n3. Tap 'Add'");
            } else if(isAndroid){
              alert("âœ“ MandaStrong Studio downloaded.\n\nTo also install it as an app:\n1. Tap the menu â‹® in Chrome\n2. Tap 'Add to Home screen' or 'Install app'");
            } else {
              alert("âœ“ MandaStrong Studio.html downloaded to your computer.\n\nDouble-click the file any time to open the studio full-screen.\n\nTo also install it: look for the install icon âŠ• in your browser's address bar.");
            }
          ...
          <div style={{color:GOLDDIM,fontSize:10,letterSpacing:2,textAlign:"center"}}>BROWSER MENU â†’ ADD TO HOME SCREEN</div>
        </div>
      </div>
    </div>
  );
...
function P2({ go }) {
  const pipeline=[{n:"01",ic:"âœ",t:"WRITE",d:"Script, logline, scenes",p:5},{n:"02",ic:"ðŸŽ™",t:"VOICE",d:"54 AI voice characters",p:6},{n:"03",ic:"ðŸŽ¨",t:"IMAGE",d:"AI-generated stills",p:7},{n:"04",ic:"ðŸŽ¬",t:"VIDEO",d:"Cinema scene engine",p:8},{n:"05",ic:"â±",t:"TIMELINE",d:"Multi-track editor",p:13},{n:"06",ic:"ðŸŽš",t:"MIX",d:"4-channel audio mixer",p:15},{n:"07",ic:"âš¡",t:"RENDER",d:"Up to 4K export",p:16}];
  const templates=[{ic:"ðŸŽ¬",t:"FEATURE FILM",d:"90-minute drama.",pages:[5,6,8,13,15,16],bg:"#1a0800"},{ic:"ðŸŽ¥",t:"DOCUMENTARY",d:"60-minute documentary.",pages:[5,6,8,13,15,16],bg:"#061a06"},{ic:"ðŸŽµ",t:"MUSIC VIDEO",d:"Beat-synced cinematic video.",pages:[6,8,13,16],bg:"#0a0618"},{ic:"ðŸŽ­",t:"SHORT FILM",d:"10-minute narrative.",pages:[5,6,8,13,16],bg:"#0a0a18"},{ic:"ðŸ‘¨â€ðŸ‘©â€ðŸ‘§",t:"FAMILY MOVIE",d:"30-minute family film.",pages:[5,6,8,13,16],bg:"#1a0a00"},{ic:"ðŸ“–",t:"AUDIOBOOK",d:"Narrated audiobook.",pages:[5,6,15,16],bg:"#181200"}];
  return(
    <div style={{...Sp,padding:"0 0 40px"}}>
      <div style={{padding:"20px 24px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div><div style={{fontSize:10,color:GOLD,letterSpacing:4,fontWeight:700,marginBottom:4}}>MANDASTRONG STUDIO Â· CINEMA INTELLIGENCE PLATFORM</div><h1 style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:"clamp(22px,4vw,40px)",fontWeight:900,letterSpacing:6,margin:0}}>STUDIO DASHBOARD</h1></div>
        ...
        <div style={{display:"flex",gap:0,overflowX:"auto"}}>
          {pipeline.map((step,i)=>(
            <div key={step.n} style={{display:"flex",alignItems:"center",flexShrink:0}}>
              ...
              {i<pipeline.length-1&&<div style={{color:GOLDDIM,fontSize:14,padding:"0 3px",flexShrink:0}}>â€º</div>}
            </div>
          ))}
        </div>
      </div>
...
function P3() {
  ...
  const handleFile=async(i,e)=>{
    ...
    try{await saveClipToDB("poc_"+i,f,f.name,f.type);}catch(err){alert("Could not save film "+(i+1)+" â€” storage may be full.");}
  };
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{color:"#22c55e",fontSize:11,marginBottom:24,letterSpacing:2,fontWeight:900}}>âœ“ SAVED PERMANENTLY â€” YOUR FILMS STAY UNTIL YOU REPLACE OR REMOVE THEM</div>
          ...
          {[0,1,2].map(i=>(
            <div key={i} style={{...Card(),padding:16}}>
              ...
              <div style={{background:"#000",aspectRatio:"16/9",marginBottom:10,border:"1px solid "+(uploads[i]?GOLD:GOLDDIM),overflow:"hidden",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}
                ...
                {uploads[i]?(
                  ...
                  <div style={{textAlign:"center",padding:16}}>
                    <div style={{color:GOLDDIM,fontSize:28,marginBottom:8}}>ðŸŽ¬</div>
                    <div style={{color:GOLD,fontSize:10,fontWeight:900,letterSpacing:2}}>CLICK TO UPLOAD</div>
                    <div style={{color:GOLDDIM,fontSize:9,marginTop:4}}>MP4 Â· WEBM Â· MOV Â· JPG Â· PNG</div>
                  </div>
                )}
                {uploads[i]&&(
              ...
              {!uploads[i]?(
                ...
                <div>
                  <div style={{color:"#22c55e",fontSize:9,fontWeight:900,letterSpacing:2,marginBottom:6}}>âœ“ {uploads[i].name.slice(0,28)} Â· {uploads[i].size}MB</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        {/* Live subscriber counter â€” green live light, auto-updates (LIVE visitors + TOTAL users) */}
        <MSUserCounter/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:18,marginBottom:36}}>
          <div style={{...Card()}}>
            ...
            {loginOk&&<div style={{background:"#061406",border:"1px solid #22c55e",padding:"10px",textAlign:"center",marginBottom:8}}>
              <span style={{color:"#22c55e",fontWeight:900,fontSize:14,letterSpacing:2}}>âœ“ LOGIN SUCCESSFUL</span>
            </div>}
            <button onClick={login} style={{...G("gold",false),width:"100%",padding:"12px"}}>{loginOk?"âœ“ ENTERING STUDIO...":"SIGN IN TO STUDIO"}</button>
          </div>
          <div style={{...Card(),border:"2px solid #22c55e",position:"relative"}}>
            <div style={{position:"absolute",top:-11,left:"50%",transform:"translateX(-50%)",background:"#22c55e",color:"#000",padding:"3px 14px",fontSize:11,fontWeight:900,whiteSpace:"nowrap"}}>ðŸŽ‰ 7-DAY FREE TRIAL</div>
            ...
            <button onClick={()=>{setUser({name:name||"Creator",plan:"Studio Trial",isAdmin:false});window.open(STRIPE.studio,"_blank");go(5);}}
              style={{width:"100%",padding:"12px",background:"#22c55e",border:"none",color:"#000",fontWeight:900,fontSize:13,cursor:"pointer",letterSpacing:2}}>START FREE TRIAL â€” $0</button>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
          {[
            ...
            <div key={plan.t} style={{...Card(),border:plan.pop?"2px solid "+GOLD:"1px solid "+GOLDDIM,position:"relative"}}>
              ...
              {plan.trial&&<div style={{position:"absolute",top:-11,right:12,background:"#22c55e",color:"#000",padding:"2px 10px",fontSize:11,fontWeight:900}}>ðŸŽ‰ FREE TRIAL</div>}
              ...
              <div style={{margin:"12px 0"}}>{plan.f.map(f=><div key={f} style={{color:WHITE,fontSize:12,padding:"3px 0",borderBottom:"1px solid #0a0a0a"}}>âœ“ {f}</div>)}</div>
        ...
        {/* â”€â”€ PURCHASE USAGE CREDITS â”€â”€ one-time top-up at the bottom of the page â”€â”€ */}
        <div style={{marginTop:40,padding:"28px 24px",background:"#050500",border:"2px solid "+GOLD,textAlign:"center",boxShadow:"0 0 24px "+GOLD+"22"}}>
          ...
          <p style={{color:WHITE,fontSize:13,lineHeight:1.7,maxWidth:520,margin:"0 auto 18px"}}>You've already had your usage replaced once. To top up again, purchase extra usage credits here â€” pay once, use them for renders and generations whenever you're ready, and they never expire. Proceeds from MandaStrong1.Etsy.com are donated to humanitarian causes.</p>
          <button onClick={()=>window.open(STRIPE.studio,"_blank")} style={{...G("gold",false),padding:"14px 44px",fontSize:14}}>ðŸ’³ PURCHASE USAGE CREDITS</button>
        </div>
      </div>
    </div>
  );
...
function MergeVideos({ onSave }) {
  ...
  const mergeAll = async () => {
    ...
    log("MandaStrong Merge Engine â€” combining "+clips.length+" videos in sequence...");
    ...
    try {
      ...
      setMergedUrl(url);
      setProgress(100);
      log("âœ“ Merge complete â€” "+(blob.size/1024/1024).toFixed(1)+"MB Â· "+clips.length+" clips combined");
      ...
      try {
        ...
        await safeSaveClipToDB(clipId, blob, fn, "video/webm");
        if(onSave) onSave({id:clipId, name:fn, type:"video/webm", url:URL.createObjectURL(blob), file:new File([blob],fn,{type:"video/webm"}), dbId:clipId});
        log("âœ“ Saved to media library â€” ready for timeline");
      } catch(e) {}
  ...
  return (
    <div>
      ...
      <button onClick={()=>fileRef.current&&fileRef.current.click()}
        ...
        â¬† ADD VIDEOS TO MERGE ({clips.length} loaded)
      </button>
      {clips.length>0&&(
        <div style={{marginBottom:10}}>
          <div style={{color:GOLD,fontSize:10,letterSpacing:2,fontWeight:900,marginBottom:6}}>DRAG TO REORDER â€” TOP = FIRST IN FILM</div>
          {clips.map((c,i)=>(
            <div key={c.id}
              draggable
              ...
              <span style={{color:GOLD,fontWeight:900,fontSize:13}}>â£¿</span>
              ...
              <div style={{display:"flex",gap:4}}>
                {i>0&&<button onClick={()=>move(i,i-1)} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:900}}>â–²</button>}
                {i<clips.length-1&&<button onClick={()=>move(i,i+1)} style={{background:"none",border:"1px solid "+GOLDDIM,color:GOLD,padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:900}}>â–¼</button>}
                <button onClick={()=>setClips(p=>p.filter((_,j)=>j!==i))} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"2px 7px",cursor:"pointer",fontSize:10,fontWeight:900}}>âœ•</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {clips.length>=2&&(
        <button onClick={mergeAll} disabled={merging}
          ...
          {merging?"âŸ³ MERGING... "+progress+"%":"âš¡ MERGE IN SEQUENCE â†’ SAVE TO TIMELINE"}
        </button>
      )}
      {merging&&(
      ...
      {mergedUrl&&(
        <div style={{background:"#061406",border:"1px solid #22c55e",padding:"10px 14px"}}>
          <div style={{color:"#22c55e",fontWeight:900,fontSize:11,letterSpacing:2,marginBottom:6}}>âœ“ MERGED FILM SAVED TO MEDIA LIBRARY â€” READY FOR TIMELINE</div>
          <a href={mergedUrl} download="MandaStrong_Merged.webm" target="_blank" rel="noopener noreferrer"
            style={{color:GOLD,fontSize:10,fontWeight:900,letterSpacing:2,textDecoration:"none"}}>â¬‡ DOWNLOAD MERGED FILM</a>
        </div>
      )}
    </div>
  );
...
function P11({ mediaLib, setMediaLib }) {
  ...
  const onFiles = async files => {
    ...
    const added=[];
    for(const f of arr){
      // Large files: warn but still allow â€” IndexedDB handles big blobs fine
    ...
    <div style={{...Sp,padding:40}}>
              ...
              {mediaLib.map(a=>(
                <div key={a.id} style={{...Card(),padding:8,position:"relative"}}>
                  {a.type.startsWith("video")&&a.url?<video src={a.url} style={{width:"100%",marginBottom:5}}/>:
                   ...
                   <div style={{height:60,background:"#000",marginBottom:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>ðŸŽµ</div>}
                  ...
                  <button onClick={()=>setMediaLib(p=>p.filter(x=>x.id!==a.id))}
                    style={{position:"absolute",top:5,right:5,background:"#7f1d1d",border:"none",color:"#ef4444",width:16,height:16,cursor:"pointer",fontSize:9,padding:0}}>âœ•</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{marginTop:20}}>
        <div style={{background:"linear-gradient(135deg,#0a0500,#1a0a00)",border:"2px solid "+GOLD,padding:16,marginBottom:12}}>
          <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:6}}>âœ¦ MERGE VIDEOS â€” COMBINE & ORDER BEFORE TIMELINE</div>
...
function P12({ go, mediaLib }) {
  const boxes=[
    {ic:"ðŸ—‚",t:"MEDIA LIBRARY",d:(mediaLib?mediaLib.length:0)+" assets",p:11},
    {ic:"â±",t:"TIMELINE EDITOR",d:"Multi-track editing",p:13},
    {ic:"âœ¨",t:"ENHANCEMENT STUDIO",d:"90+ AI tools",p:14},
    {ic:"ðŸŽµ",t:"AUDIO MIXER",d:"4-channel mixing",p:15},
    {ic:"âš¡",t:"RENDER ENGINE",d:"Up to 8K output",p:16},
    {ic:"â–¶",t:"PREVIEW PLAYER",d:"Full-screen playback",p:17}
  ];
  return (
    ...
    <div style={{...Sp,padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>{
            // Auto-populate tracks from media library and sync â€” sorted in correct order
            ...
            setTimeline(newTl);
            // â”€â”€ DURATION GAP CHECK â€” works across the whole slider, any length â”€â”€
            ...
            if(gapSec>30){
              ...
              if(wantFill){
                ...
                alert("âœ“ Fill-scene prompt copied.\n\nGo to Page 8 (Video Tools), paste it, and generate "+n+" more clip"+(n>1?"s":"")+" â€” then SYNC again.");
                go(8);
                return;
              }
            }
            alert("âœ“ All tracks synced in order â€” "+videoAssets.length+" video clips Â· "+audioAssets.length+" audio tracks");
          }} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"5px 14px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>âš¡ SYNC ALL TRACKS</button>
          <button onClick={()=>go(16)} style={{...G("gold",false)}}>â†’ RENDER</button>
          <button onClick={()=>go(11)} style={{...G("out",true)}}>â¬† UPLOAD MEDIA</button>
            ...
            {(timeline[idx]||[]).map((a,i)=>(
              <div key={i} draggable
                ...
                <button onClick={()=>setTimeline(p=>({...p,[idx]:p[idx].filter((_,j)=>j!==i)}))}
                  style={{background:"none",border:"none",color:"#000",cursor:"pointer",fontSize:11,padding:0}}>âœ•</button>
              </div>
            ))}
      ...
      <div style={{...Card(),marginTop:12,display:"flex",alignItems:"center",gap:8}}>
        {["â®","âª","â–¶","â©","â­"].map(c=><button key={c} style={{...G("out",true)}}>{c}</button>)}
...
function P16({ go, timeline, setRendered, mediaLib, setMediaLib, user, filmDuration, setFilmDuration }) {
  ...
  // â”€â”€ GAP-FILL CHOICE (Y = generate extra scenes, N = stretch clips) â”€â”€
  ...
  const getAudioTrack=()=>{
    ...
    // to a preset voice â€” which is exactly why your voice never continued.
  ...
  const startRender=async()=>{
    ...
    // â”€â”€ PRE-RENDER STORAGE CHECK â€” never touches source clips â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try{
      ...
      const oldRenders=clips.filter(c=>String(c.id).includes("render_final_old"));
      for(const c of oldRenders){await deleteClipFromDB(c.id);}
      log("Memory check complete â€” "+clips.length+" clips preserved");
    }catch(e){}

    // â”€â”€ CLIP ORDER: the TIMELINE is the authority â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // filename â€” with names like GODS_GURUS_6 and LOVE_WAR_60s that number is
    ...
    // â”€â”€ EXCLUDE old rendered films and empty clips â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // A previously-rendered "MandaStrong_Film..." file in the library has no real
    // scene frames â€” including it makes the whole render come out 0.0MB.
    ...
    if(!hasTimeline){
      ...
      log("No timeline â€” clips ordered by scene number");
    } else {
      log("Render order locked to timeline: "+clips.map(c2=>(c2.name||"").slice(0,18)).join(" â†’ "));
    }
    ...
    try{
      ...
      if(audioAsset){
        // â”€â”€ CLONED-VOICE FULL NARRATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if(audioAsset.clonedVoiceId&&audioAsset.narrText){
          try{
            ...
            if(decoded.length){
              ...
              audioBuffer=merged;
              log("âœ“ Full narration baked in your cloned voice: "+total.toFixed(1)+"s");
            }
          }catch(e){log("Cloned-voice narration error â€” trying the recording: "+e.message);}
          ...
          if(!audioBuffer){
            try{
              ...
              if(audioBlob){const arrayBuf=await audioBlob.arrayBuffer();audioBuffer=await audioCtx.decodeAudioData(arrayBuf);log("âœ“ Played your recording instead: "+audioBuffer.duration.toFixed(1)+"s");}
            }catch(e){}
          }
        } else
        ...
        // Fix: fetch real audio from engineSpeak, decode into THIS audioCtx, feed audioDest â€”
        ...
        if(audioAsset.type==="narration"||(!audioAsset.url&&!audioAsset.file&&audioAsset.text)){
          try{
            ...
            log("Baking narration through Cinema Voice Engine â€” "+narrChunks.length+" segment(s)...");
            const decoded=[];
            ...
            if(decoded.length){
              ...
              for(const b of decoded){ out.set(b.getChannelData(0),Math.floor(off*audioCtx.sampleRate)); off+=b.duration; }
              audioBuffer=merged;
              log("âœ“ Narration baked from Cinema Voice Engine: "+total.toFixed(1)+"s");
            } else {
              liveNarration=true;
              log("Engine narration unavailable â€” speaking live as fallback");
            }
          }catch(e){
            liveNarration=true;
            log("Narration bake error â€” live fallback: "+e.message);
          }
        } else {
          try{
            let audioBlob=null;
            ...
            if(audioBlob){
              ...
              log("âœ“ Audio loaded: "+(audioBuffer.duration).toFixed(1)+"s");
            } else {
              log("Audio asset found but no data â€” video only");
            }
          }catch(e){log("Audio load failed: "+e.message+" â€” video only");}
        }
      }
      ...
      // â”€â”€ BACKGROUND MUSIC BED â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      // Plays under the narration, quiet, looped to cover the whole film. Voice
      // stays on top (locked mix VOICE 85 / MUSIC 40 â‰ˆ 0.25 gain under voice).
      ...
      if(!vTrack||vTrack.readyState!=="live"){
        log("âš  Canvas capture unavailable in this browser.");
      ...
      // â”€â”€ ADAPTIVE BITRATE â€” caps total memory so long films finish encoding â”€â”€
      ...
      if(bitrate<requested)log("Adaptive bitrate: "+(bitrate/1000000).toFixed(1)+"Mbps for "+Math.round(totalFilmSec)+"s film â€” keeps memory safe to the end");
      ...
      if(liveNarration&&audioAsset?.text){
        ...
        log("âœ“ Speaking narration live (fallback): "+(audioAsset.voice||"blaze"));
      }
      ...
      // â”€â”€ GAP-FILL: the DURATION SLIDER is master â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      // filmDuration (1â€“180 min, set on the timeline page) decides the film length.
      // Clips stretch to fill that total: each clip holds (sliderSecs / clipCount).
      // The old 65s-per-clip cap is lifted â€” the engine accepts long clips, so a
      ...
      if(targetTotal>0 && clips.length>0){
        if(gapFill){
          ...
          if(gap > 5){
            ...
            log("Fill-in ON â€” film built from "+clips.length+" scenes (real + generated)");
          } else {
            log("Fill-in ON â€” footage already covers the target, nothing to add");
          }
        } else {
          perClipTarget = Math.max(targetTotal / clips.length, 3);
          log("Stretch mode: film "+(targetTotal/60).toFixed(1)+" min Ã· "+clips.length+" clips â‰ˆ "+perClipTarget.toFixed(1)+"s each");
        }
      }
      ...
      for(let ci=0;ci<clips.length;ci++){
        ...
        // If video failed or no file â€” regenerate scene with Claude
        if(!videoPlayed){
          log("  Clip not playable â€” generating scene: "+clip.name.slice(0,30)+"...");
          ...
          if(!ok){
            // Last resort: plain black hold â€” real-time paced. No words on screen;
      ...
      const blob=new Blob(chunks,{type:mimeType});
      // â”€â”€ SAFETY: never hand an empty file to the player (that's the grey arrow) â”€â”€
      if(!chunks.length||blob.size<10000){
        log("âš  RENDER PRODUCED NO VIDEO DATA");
        log("Your browser blocked canvas capture. Fix: keep this tab in front");
        log("for the whole render, and try 720p Â· 24FPS.");
        setProgress(0);setDone(false);setRendering(false);
        try{clearInterval(dataInterval);}catch(e){}
        try{clearInterval(heartbeat);}catch(e){}
        try{if(audioCtx)audioCtx.close();}catch(e){}
        alert("Render produced no video data.\n\nKeep this tab in front for the whole render (don't switch apps or tabs), and use 720p Â· 24FPS. Then try again.");
        return;
      }
      const url=URL.createObjectURL(blob);
      setRenderUrl(url);
      ...
      log("RENDER COMPLETE â€” "+(blob.size/1024/1024).toFixed(1)+"MB");
      // Save final render to IndexedDB â€” timeout-protected, never blocks completion
      try{
  ...
  const QUALITIES=[{id:"480p",label:"480p",sub:"854Ã—480"},{id:"720p",label:"720p",sub:"1280Ã—720"},{id:"1080p",label:"1080p",sub:"1920Ã—1080"},{id:"4K",label:"4K",sub:"3840Ã—2160"}];
  ...
  return (
    <div style={{...Sp,padding:0}}>
      ...
      <div style={{padding:"12px 24px",borderBottom:"1px solid "+GOLDDIM+"",background:"#020200",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:10,color:GOLD,letterSpacing:4,fontWeight:700}}>PRODUCTION ENGINE â€” STAGE 6</div>
        ...
        <div style={{padding:20,overflowY:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            <div style={{background:clips.length>0?"#061406":"#0a0a0a",border:"1px solid "+(clips.length>0?"#22c55e":GOLDDIM),padding:"14px 16px"}}>
              ...
              <div style={{color:clips.length>0?"#22c55e":WHITE,fontSize:14,fontWeight:900}}>{clips.length>0?"âœ“ "+clips.length+" clip"+(clips.length>1?"s":"")+" ready":"No clips â€” generate on page 8"}</div>
            </div>
            <div style={{background:audio?"#061406":"#0a0a0a",border:"1px solid "+(audio?"#22c55e":GOLDDIM),padding:"14px 16px"}}>
              ...
              <div style={{color:audio?"#22c55e":"#f59e0b",fontSize:14,fontWeight:900}}>{audio?"âœ“ Audio ready":"No audio â€” record on page 6"}</div>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          ...
          {renderLog.length>0&&(
            <div style={{background:"#000",border:"1px solid "+GOLDDIM,padding:"14px 16px",marginBottom:16,maxHeight:180,overflowY:"auto"}}>
              ...
              {renderLog.map((l,i)=>(
                <div key={i} style={{color:i===renderLog.length-1?"#22c55e":"#666",fontSize:10,lineHeight:1.7,fontFamily:"monospace"}}>{i===renderLog.length-1?"â–º ":"  "}{l}</div>
              ))}
            </div>
          )}
          {done&&renderUrl&&(
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                <button onClick={()=>{
                  try{
                    ...
                    log("âœ“ Download started â€” check your device's Downloads");
                  }catch(e){ try{window.open(renderUrl,"_blank");}catch(e2){} log("Opened film in new tab â€” long-press to save"); }
                }} style={{...G("gold",false),padding:"12px 24px",fontSize:12,letterSpacing:2,cursor:"pointer"}}>â¬‡ DOWNLOAD FILM</button>
          ...
          {/* â”€â”€ FILL IN THE GAPS? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          ...
          <div style={{background:"#050500",border:"2px solid "+GOLD,padding:"18px 20px",marginBottom:16}}>
            <button onClick={startRender} disabled={rendering||clips.length===0}
              ...
              {rendering?"RENDERING... "+progress+"%":"START RENDER â€” "+quality+" Â· "+fps+"fps Â· "+clips.length+" CLIP"+(clips.length!==1?"S":"")}
            </button>
          </div>
          <div style={{display:"flex",gap:8}}>
          ...
          <div style={{background:"#000",aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
            {renderUrl?(
              ...
              <div style={{textAlign:"center",padding:20}}>
                ...
                <div style={{color:DIM,fontSize:10,lineHeight:1.8}}>{quality} Â· {fps}fps<br/>{clips.length} clip{clips.length!==1?"s":""} queued</div>
              </div>
            )}
          </div>
          <div style={{flex:1,overflowY:"auto",padding:14}}>
      ...
      <div style={{maxWidth:880,margin:"0 auto"}}>
        ...
        <div style={{background:"#000",overflow:"hidden",marginBottom:14,aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid "+GOLDDIM}}>
          {vs?<video ref={videoRef} src={vs} style={{width:"100%",height:"100%"}} controls
            ...
            <div style={{textAlign:"center",color:GOLDDIM,fontSize:40}}>ðŸŽ¬</div>}
        </div>
        <div style={{...Card(),display:"flex",alignItems:"center",gap:8}}>
          <button onClick={()=>{if(videoRef.current)videoRef.current.currentTime=0;}} style={{...G("out",true)}}>â®</button>
          <button onClick={()=>{if(videoRef.current)videoRef.current.currentTime-=10;}} style={{...G("out",true)}}>âª</button>
          <button onClick={togglePlay} style={{...G("gold",true),minWidth:44}}>{isPlaying?"â¸":"â–¶"}</button>
          <button onClick={()=>{if(videoRef.current)videoRef.current.currentTime+=10;}} style={{...G("out",true)}}>â©</button>
...
function P18({ rendered, mediaLib }) {
  ...
  const dl=()=>{if(!vs){alert("No film yet â€” render first!");return;}const a=document.createElement("a");a.href=vs;a.download="MandaStrong_Film.webm";a.target="_blank";a.rel="noopener noreferrer";a.click();};
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:780,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
          {[["ðŸ’¾","DOWNLOAD TO DEVICE",dl],["ðŸ’¿","SAVE PROJECT FILE",()=>{}],["ðŸŒ","SHARE TO COMMUNITY",()=>{}]].map(([ic,lb,fn])=>(
...
function TutCanvas({drawFn}){
  ...
  useEffect(()=>{
    ...
    const draw=(ts)=>{
      ...
      // Base â€” Claude animation
      try{drawFn(ctx,W,H,t,sec);}
      catch(e){ctx.fillStyle="#050300";ctx.fillRect(0,0,W,H);}

      // â”€â”€ CINEMATIC POLISH LAYER â€” makes every tutorial feel premium â”€â”€â”€â”€

      // Warm cinematic grade â€” subtle gold tint over entire frame
      ...
      // Highlight recovery â€” glow near top centre
      const hr=ctx.createRadialGradient(W/2,H*0.3,0,W/2,H*0.3,W*0.4);
      hr.addColorStop(0,"rgba(255,245,215,0.06)");hr.addColorStop(1,"rgba(0,0,0,0)");
      ctx.fillStyle=hr;ctx.fillRect(0,0,W,H);

      // Ambient particle depth â€” slow-drifting gold specks
      particles.forEach(p=>{
      ...
      // Vignette â€” cinematic edge fall-off
      ...
      // Letterbox bars â€” 2.35:1 cinema look
      ...
      // Professional branding overlay â€” top-left studio mark
      ...
      // Bottom right â€” TUTORIAL watermark
      ctx.font="bold "+Math.round(H*0.017)+"px Georgia, serif";
      ctx.textAlign="right";
      ctx.fillStyle="rgba(232,201,109,0.55)";
      ctx.fillText("â€¢ TUTORIAL â€¢",W-bar-10,H-bar-Math.round(H*0.025));
...
function P19({ go }) {
  ...
  const tuts=[
    {n:"01",t:"Getting Started â€” Platform Overview",d:"Complete walkthrough of all 24 pages, Quick Access menu, footer controls, auto-save, and navigation.",dur:"3:00",l:"Beginner",page:1,tips:["Use â˜° top left to jump to any page","AUTOSAVE ON is real â€” state saves automatically as you work","ðŸ’¾ SAVE PROJECT saves a named session to MY PROJECTS for full restore"]},
    {n:"02",t:"Writing Tools â€” Script to Screen",d:"How to use the 100+ writing tools on Page 5. From logline to full feature script. All results auto-save to Media Library.",dur:"4:00",l:"Beginner",page:5,tips:["Click any tool card to open it","Use AI CREATE for instant professional scripts","Save results to your Media Library â€” they auto-route to the timeline"]},
    {n:"03",t:"Voice Engine â€” 54 Characters",d:"Selecting voices, filtering by gender, age, and origin. Recording narration. Two-button save workflow.",dur:"5:00",l:"Beginner",page:6,tips:["Hit PREPARE TO SPEAK to hear your narration aloud","Hit SAVE TO MEDIA LIBRARY to save it â€” auto-adds to timeline audio track","Filter by gender, age, and origin to find the perfect voice for your project"]},
    {n:"04",t:"Music Video Studio â€” Full Walkthrough",d:"Step-by-step: Song setup, style selection, scene description, drag-and-drop audio upload, generating and exporting.",dur:"5:00",l:"Intermediate",page:6,tips:["Access from MUSIC VIDEO STUDIO button on Page 6","Drag and drop your audio file onto the upload zone â€” or click to browse","Record your own song with the red RECORD button"]},
    {n:"05",t:"Video Generator â€” Cinematic Scenes",d:"Describe any scene and have the Cinema Engine build it. Upload reference photos for photoreal output. Auto-saves to library and timeline.",dur:"4:00",l:"Intermediate",page:8,tips:["Upload a reference photo FIRST â€” engine builds the scene around it","Be specific: lighting, mood, camera angle, time of day","Generated clips save automatically to Media Library and Timeline"]},
    {n:"06",t:"Timeline Editor â€” Building Your Film",d:"Clips auto-populate from Media Library. Drag to reorder. Upload Media button always visible. SYNC ALL TRACKS for instant assembly.",dur:"4:00",l:"Intermediate",page:13,tips:["Hit âš¡ SYNC ALL TRACKS to auto-populate all clips in order","Use â¬† UPLOAD MEDIA (next to CLEAR ALL) to add more clips at any time","Narration saves auto-populate the audio track â€” no dragging needed"]},
    {n:"07",t:"Audio Mixer â€” Professional Sound",d:"Setting the perfect mix for documentary, narrative film, or music video.",dur:"3:00",l:"Beginner",page:15,tips:["Documentary: VOICE 85 Â· MUSIC 40 Â· EFX 50 Â· MASTER 85","Music Video: MUSIC 75 Â· VOICE 60 Â· EFX 40 Â· MASTER 85","Hit Apply Mix when done before going to Page 16"]},
    {n:"08",t:"Render Engine â€” 4K with Auto-Enhancement",d:"Quality settings 480p to 4K. Auto-enhancement runs on every frame â€” contrast, colour grade, sharpness, noise reduction. Priority save protects your work before render starts.",dur:"4:00",l:"Intermediate",page:16,tips:["Auto-enhancement runs automatically â€” no settings needed","Priority save fires before render starts so a crash never loses your session","4K recommended for professional distribution â€” 1080p for social media"]},
    ...
    {n:"10",t:"Saving & Project History",d:"Real auto-save keeps your work safe at all times. Emergency crash save fires if the tab closes. Named sessions in MY PROJECTS for full restore.",dur:"2:00",l:"Beginner",page:1,tips:["AUTOSAVE ON is real â€” saves every time you change page, timeline, or media","ðŸ’¾ SAVE PROJECT creates a named restore point in MY PROJECTS","ðŸ“‚ MY PROJECTS â†’ CONTINUE PROJECT restores your full session including clips"]},
    {n:"11",t:"Character Studio â€” Page 24",d:"Create and save reusable characters with reference photos, voice assignments, and appearance notes. Use in any scene.",dur:"3:00",l:"Intermediate",page:24,tips:["Upload a reference photo for each character","Assign a voice from the 54-character library","Hit USE IN SCENE to send the character to your Media Library"]},
    {n:"12",t:"Documentary Workflow â€” Full Case Study",d:"Complete end-to-end documentary production: script to 4K render. 13 scenes, narration, timeline assembly, and export.",dur:"5:00",l:"Advanced",page:8,tips:["Page 5 â†’ paste director instructions + full narration script into Script to Movie","Page 6 â†’ select your voice â†’ PREPARE TO SPEAK â†’ SAVE TO MEDIA LIBRARY","Page 8 â†’ generate all scenes â†’ Page 13 â†’ Sync â†’ Page 16 â†’ Render 4K"]},
  ];
    ...
    try{
      const res=await fetch("https://njqfexhltjwpgvctmyaw.supabase.co/functions/v1/claude-proxy",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:4000,
          messages:[{role:"user",content:"Write a highly polished JavaScript canvas animation for a professional cinema-platform tutorial about \""+t.t+"\". This is for MandaStrong Studio â€” the aesthetic must be premium: gold (#e8c96d) on deep black, glowing highlights, smooth eased motion using Math.sin() and clean easing curves, deep drop shadows, professional serif and sans typography. Animation should include: (1) an elegant animated LESSON "+t.n+" number card that fades in and settles, (2) the tutorial title \""+t.t+"\" appearing letter by letter with warm gold glow, (3) an animated diagram or visual metaphor that illustrates the topic conceptually â€” smooth transitions, layered shapes, glowing lines, not stick figures, (4) key concepts revealed one at a time with smooth fade-in, (5) subtle particle effects and ambient movement to avoid static frames. Every frame should look like a high-end motion graphics piece â€” think Apple keynote crossed with cinema title design. Use sec for continuous animation, t=0-1 for progress. W=canvas width, H=canvas height. Do not draw letterbox bars, watermarks, or vignettes â€” those are added separately. Return ONLY: function drawFrame(ctx,W,H,t,sec){"}]})
      });
      const d=await res.json();
      let code=d.content&&d.content[0]?d.content[0].text.trim():"";
              ...
              <div style={{background:isActive?"#060400":"#030200",border:"1px solid "+(isActive?GOLD:GOLDDIM+"66"),borderBottom:isActive?"none":undefined,display:"flex",alignItems:"stretch",cursor:"pointer"}}
                ...
                <div style={{flex:1,padding:"13px 16px",minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:3}}>
                    ...
                    {hasFn&&<span style={{background:"#22c55e1a",border:"1px solid #22c55e",color:"#22c55e",padding:"1px 8px",fontSize:9,fontWeight:900,letterSpacing:2}}>GENERATED âœ“</span>}
                  </div>
                  <div style={{color:GOLDDIM,fontSize:10,letterSpacing:1}}>{t.dur} Â· {t.tips.length} PRO TIPS</div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10,padding:"0 16px",flexShrink:0}}>
                  {isGen?(
                    ...
                    <button onClick={e=>{
                      e.stopPropagation();
                      if(hasFn){
                        // PLAY AGAIN â€” re-narrate with Blaze
                        try{window.speechSynthesis.cancel();}catch{}
                        const narration="Lesson "+parseInt(t.n)+". "+t.t+". "+t.d+" Pro tips. "+t.tips.join(". ")+".";
                      ...
                      {hasFn?"â–¶ PLAY AGAIN":"â–¶ GENERATE TO WATCH"}
                    </button>
                  )}
                  <span style={{color:isActive?GOLD:GOLDDIM,fontSize:14,fontWeight:900}}>{isActive?"â–²":"â–¼"}</span>
                </div>
              </div>
              {isActive&&(
                  ...
                  {isGen?(
                    ...
                    <div style={{aspectRatio:"16/9",background:"linear-gradient(135deg,#060400,#020100)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18,cursor:"pointer"}}
                      ...
                      <div style={{width:80,height:80,background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 50px "+GOLD+"55",cursor:"pointer"}}>
                        <span style={{color:"#000",fontSize:30,fontWeight:900,marginLeft:4}}>â–¶</span>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{color:GOLD,fontWeight:900,fontSize:15,letterSpacing:4,marginBottom:6}}>GENERATE TO WATCH</div>
                        <div style={{color:GOLDDIM,fontSize:10,letterSpacing:2}}>LESSON {t.n} Â· {t.dur} Â· {t.l.toUpperCase()}</div>
                      </div>
                    </div>
                  )}
                  <div style={{padding:"20px 26px"}}>
                    ...
                    {t.tips.map((tip,i)=>(
                      <div key={i} style={{display:"flex",gap:12,marginBottom:9,alignItems:"flex-start"}}>
                        <span style={{color:GOLD,fontWeight:900,flexShrink:0}}>âœ¦</span>
                        <span style={{color:WHITE,fontSize:13,lineHeight:1.75}}>{tip}</span>
                      </div>
                    ))}
                    <div style={{marginTop:18,display:"flex",gap:10,flexWrap:"wrap"}}>
                      {!hasFn&&!isGen&&<button onClick={()=>generate(idx)} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"11px 28px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>â–¶ GENERATE TO WATCH</button>}
                      {hasFn&&!isGen&&<button onClick={()=>generate(idx)} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"11px 28px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>â†º REGENERATE</button>}
                      <button onClick={()=>go(t.page)} style={{background:"transparent",border:"1px solid "+GOLD,color:GOLD,padding:"11px 20px",cursor:"pointer",fontSize:11,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>OPEN PAGE {t.page} â–¶</button>
                    </div>
                  </div>
                </div>
              )}
  ...
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:860,margin:"0 auto"}}>
        ...
        <div style={{color:WHITE,fontSize:11,marginBottom:20,letterSpacing:2}}>EFFECTIVE MARCH 2026 Â· MANDASTRONG STUDIO</div>
        ...
        {tab==="tos"&&(
          <div>
            <div style={{background:"#050500",border:"2px solid "+GOLD,padding:"14px 20px",marginBottom:20,textAlign:"center"}}>
              <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900}}>MANDASTRONG STUDIO Â· PROFESSIONAL CINEMA INTELLIGENCE PLATFORM</div>
            ...
            {sec("6. SOCIAL MISSION",<>{p("A meaningful portion of all subscription proceeds is donated to veterans mental health initiatives and school anti-bullying programmes. These are not marketing statements â€” they are the founding mission of this platform. Full details available at MandaStrong1.Etsy.com.")}</>)}
            ...
            <div style={{background:"#050500",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginTop:8}}>
              <p style={{color:GOLDDIM,fontSize:11,margin:0,letterSpacing:1}}>MANDASTRONG STUDIO Â· AMANDA WOOLLEY, FOUNDER Â· MARCH 2026</p>
            </div>
          </div>
        )}
        ...
        {tab==="disc"&&(
          <div>
            <div style={{background:"#050500",border:"2px solid "+GOLD,padding:"14px 20px",marginBottom:20,textAlign:"center"}}>
              <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900}}>IMPORTANT â€” PLEASE READ BEFORE USING THIS PLATFORM</div>
            ...
            <div style={{background:"#050500",border:"1px solid "+GOLDDIM,padding:"12px 16px",marginTop:8}}>
              <p style={{color:GOLDDIM,fontSize:11,margin:0,letterSpacing:1}}>â€” AMANDA WOOLLEY Â· FOUNDER Â· MANDASTRONG STUDIO Â· MARCH 2026 Â· mandastrong01.bolt.host</p>
            </div>
          </div>
        )}
      </div>
...
function P21() {
  const [msgs,setMsgs]=useState([{role:"assistant",content:"Welcome to MandaStrong Studio. I am Agent Grok â€” your 24/7 production consultant. Ask me anything about tools, workflow, pricing, or filmmaking."}]);
...
function P22() {
  const [posts,setPosts]=useState([{id:1,user:"Sarah J.",title:"Epic Action Feature",icon:"ðŸŽ¬",views:2847,likes:1522},{id:2,user:"Mike Chen",title:"Family Documentary",icon:"ðŸ“½",views:1256,likes:812},{id:3,user:"Emily R.",title:"Short Film Entry",icon:"ðŸ†",views:3421,likes:2156},{id:4,user:"Alex T.",title:"Music Video Cut",icon:"ðŸŽµ",views:5234,likes:4012}]);
  return (
    <div style={{...Sp,padding:40}}>
      <div style={{maxWidth:780,margin:"0 auto"}}>
        ...
        {posts.map(p=>(
          <div key={p.id} style={{...Card(),marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <span style={{color:WHITE,fontSize:12}}>ðŸ‘ {p.views.toLocaleString()}</span>
              <span style={{color:WHITE,fontSize:12}}>â¤ï¸ {p.likes.toLocaleString()}</span>
...
function HowToGuide() {
  const [open,setOpen]=useState(null);
  const SECTIONS=[
    {t:"WELCOME â€” HOW TO READ THIS BOOK",c:"This is more than a how-to. It is a complete guide to making films with AI on MandaStrong Studio (mandastrong01.bolt.host) AND a plain-English education in what AI actually is, so you are never at its mercy. Read Part One to understand the machine you are working with. Read Part Two to master the studio page by page. Read Part Three for the craft â€” prompting, voice, story, and ethics. You do not need any technical background. Every idea here is explained the way you would explain it to a friend across a kitchen table."},

    {t:"PART ONE Â· WHAT AI ACTUALLY IS",c:"AI does not think, feel, or know things the way you do. A large language model â€” the kind of AI behind most creative tools â€” is a very powerful pattern machine. It has read an enormous amount of human writing and images and learned which words and shapes tend to follow which. When you ask it for something, it is not looking up an answer; it is predicting, piece by piece, the most likely continuation of your request. That is why it can sound confident and still be wrong. Understanding this one fact changes how you use it: you are the director, it is the crew. It is fast and tireless and knows a thousand styles, but it has no judgement about YOUR story. That judgement is yours, and it always will be."},

    {t:"PART ONE Â· WHY AI SOMETIMES GETS IT WRONG",c:"Because AI predicts rather than knows, it can 'hallucinate' â€” state something untrue with total confidence, invent a fact, or misread what you wanted. This is normal and expected, not a fault in you. The fix is always the same: be more specific, give an example, or break the task into smaller steps. If a render or a line of narration comes out wrong, it is almost never because you did something stupid â€” it is because the machine guessed and guessed poorly. Re-roll it, refine your wording, and move on. Treat every output as a first draft from a talented but literal-minded assistant."},

    {t:"PART ONE Â· PROMPTING â€” TALKING TO THE MACHINE",c:"A prompt is simply your instruction to the AI. The single biggest skill in the whole studio is learning to prompt well, and it comes down to specificity. 'A man walking' gives the machine nothing to hold onto. 'A weathered fisherman in his sixties walking along a stormy grey beach at dawn, wind pulling at his yellow raincoat, shot from behind, cinematic, muted cold colour grade' gives it everything. Name the subject, the setting, the light, the mood, the camera angle, and the style. Show, don't summarise. When in doubt, describe it as if to someone who cannot see what is in your head â€” because that is exactly the situation."},

    {t:"PART ONE Â· AI AND YOU â€” STAYING IN CHARGE",c:"AI is a tool, like a camera or a pen. It amplifies whoever holds it. It has no taste of its own, so your taste is the whole game. Never let a machine talk you out of a creative instinct, and never assume its confident answer is correct without checking. Keep your own copies of everything important. Understand that what you type may be processed on servers you don't control, so don't paste anything you'd be uncomfortable sharing. And remember the deeper point behind this whole studio: AI should widen the door to creativity, not replace the human standing in it. You are not being replaced. You are being equipped."},

    {t:"PART TWO Â· GETTING STARTED",c:"Open mandastrong01.bolt.host. Log in with your credentials or start a free trial. Use the â˜° hamburger menu top left to jump to any of the 24 pages. AUTOSAVE ON is real â€” your work saves automatically every time you change page, generate a clip, or update your timeline. Hit ðŸ’¾ SAVE PROJECT to create a named restore point you can return to from MY PROJECTS. Your plan and remaining usage are always visible from your account panel â€” tap the avatar top right."},

    {t:"PART TWO Â· PAGE 1 â€” HOME & INSTALL",c:"The front door of mandastrong01.bolt.host. The DOWNLOAD APP button installs the studio to your device like a real app, using your browser's built-in install prompt â€” on iPhone and iPad use Share then Add to Home Screen, as Apple does not allow one-tap install. The whole page is built to fit any screen, phone or laptop. From here, enter the studio and begin."},

    {t:"PART TWO Â· PAGE 4 â€” PLANS & USAGE CREDITS",c:"Three plans: Basic $20, Pro $30, Studio $50 â€” pick the one that fits how much you create. At the very bottom is PURCHASE USAGE CREDITS: a one-time top-up for extra renders and generations when you need more than your plan includes. Credits never expire. All payments run through Stripe's secure checkout â€” the studio never sees your card details."},

    {t:"PART TWO Â· PAGE 5 â€” WRITING TOOLS & SCRIPT TO MOVIE",c:"100+ AI writing tools. Type a description into any tool and hit AI CREATE for professional results. At the top sits SCRIPT TO MOVIE â€” three boxes: PRODUCER (your vision, tone, casting), DESCRIBE (the film scene by scene), and PRODUCTION NOTES (shots, camera, lighting, locations). Fill them, save each to your Media Library, then hit WIRE INTO RENDER. From that moment the video generator on Page 8 uses your three boxes to drive every scene it makes. This is how your written vision becomes moving pictures."},

    {t:"PART TWO Â· PAGE 6 â€” VOICE ENGINE",c:"Cinematic voices that sound human, not robotic. Filter by gender, age, and origin. Tap any voice card to hear it. Paste your narration into the text box and hit PREPARE TO SPEAK to hear it in your chosen voice with your exact speed, pitch, and mood settings. RECORD MY VOICE lets you narrate yourself; UPLOAD lets you bring in a recording. On the right, USE MY VOICE AS NARRATION turns your own recording into the film's narration, with a duration option. Hit SAVE TO MEDIA LIBRARY and the narration auto-adds to your timeline's audio track. The MUSIC VIDEO STUDIO button lives up top."},

    {t:"PART TWO Â· PAGE 8 â€” VIDEO GENERATOR",c:"The heart of the studio. Upload a reference photo first and the engine builds the scene around your real image for photorealistic output. Paste your scene prompt â€” or let your wired Script to Movie brief drive it â€” pick your options, and hit GENERATE SCENE. You can add background music from the built-in library with a live preview, and toggle USE STEREO SOUND for a full stereo mix. Clips save automatically and populate your timeline's video track. Generate all your scenes, then move to the timeline."},

    {t:"PART TWO Â· THE TIMELINE, MIX & RENDER",c:"Page 13: hit âš¡ SYNC ALL TRACKS to lay your clips and narration in order â€” drag to reorder. Page 15: set your audio mix so voice sits above music. Page 16: choose quality up to 4K and render â€” auto-enhancement (warm grade, contrast, sharpness) runs on every frame, and an emergency save fires before rendering so a crash never costs you the session. Do not close the tab while rendering. Page 17 previews the finished film; Page 18 exports and shares to YouTube, Instagram, TikTok, Facebook, X, and Vimeo."},

    {t:"PART TWO Â· MUSIC VIDEO STUDIO",c:"Open from Page 6, top right. SONG step: enter title, artist, genre, mood, tempo; drag and drop your audio, click to upload, or hit RECORD YOUR OWN SONG; toggle USE STEREO SOUND. The DURATION slider runs freely from AUTO up to 180 minutes â€” at AUTO the film matches your song's length exactly; drag it up to lock a fixed length that overrides the music. STYLE step: video look, colour grade, effects. SCENE step: describe what we see. GENERATE builds a full beat-synced video. Download or save when done."},

    {t:"PART TWO Â· PAGE 24 â€” CHARACTER STUDIO",c:"Build reusable characters. Drag and drop a reference photo or tap to choose one, assign a voice, and record appearance notes â€” hair, eyes, costume, personality, role. Save the character and reuse them across every scene so your cast stays consistent. Characters persist across sessions. From here, HOME returns to the studio and EXIT APP signs you out."},

    {t:"PART THREE Â· THE CRAFT OF PROMPTING FOR FILM",c:"For video, think like a cinematographer. Always specify five things: subject (who or what), setting (where and when), light (dawn, neon, candlelight), motion (what moves and how), and style (film stock, colour grade, mood). Add a camera instruction â€” wide establishing shot, slow push-in, handheld, aerial. Contradictions confuse the machine, so keep your prompt coherent. If a scene comes out flat, add sensory and lighting detail rather than more objects. Consistency across scenes comes from reusing the same style language every time."},

    {t:"PART THREE Â· WRITING NARRATION THAT LANDS",c:"Narration is written to be heard, not read. Short sentences. Natural breath points. Read every line aloud before you save it â€” if you stumble, the voice engine will too. Punctuation is your friend: a full stop is a real pause, a comma a small one. Match the voice to the story â€” a documentary wants warmth and authority; a thriller wants restraint. Use the PREPARE TO SPEAK button as your rehearsal room, and tune speed and pitch until it feels like a person, not a reader."},

    {t:"PART THREE Â· STORY FIRST, ALWAYS",c:"The most photorealistic render in the world means nothing without a reason to watch. Decide what your film is really about before you generate a single frame â€” the feeling you want to leave behind. Use the Script to Movie PRODUCER box to write that down and keep yourself honest. AI can make anything look good; only you can make it mean something. Structure beats spectacle. A clear beginning, a turn in the middle, and an earned ending will carry a simple film further than dazzling clips with no spine."},

    {t:"PART THREE Â· ETHICS & RESPONSIBILITY",c:"With these tools you can make almost anything, which means the responsibility is yours. Don't put real people's faces or voices into films they never agreed to. Be honest when something is AI-generated if presenting it as real could mislead. Respect others' work rather than copying a living artist's style wholesale and calling it your own. And remember MandaStrong's founding mission â€” these tools exist to spread kindness, understanding, and hope, with proceeds supporting veterans' mental health and anti-bullying work. Make things that would make that mission proud."},

    {t:"SAVING, RECOVERING & GETTING HELP",c:"AUTOSAVE ON saves as you work. ðŸ’¾ SAVE PROJECT creates a named session â€” name it meaningfully. ðŸ“‚ MY PROJECTS shows your history; CONTINUE PROJECT restores a session including all clips. An emergency save fires if the tab closes or crashes, so work is never permanently lost. Stuck? Agent Grok on Page 21 is your 24/7 production consultant with full knowledge of every page and workflow. This guide lives on your closing page at mandastrong01.bolt.host and is updated as the studio grows."},

    {t:"RECOMMENDED WORKFLOW â€” START TO FINISH",c:"Page 5 â†’ fill Script to Movie's Producer, Describe, Production boxes â†’ WIRE INTO RENDER. Page 6 â†’ choose a voice â†’ PREPARE TO SPEAK â†’ SAVE TO MEDIA LIBRARY. Page 8 â†’ upload a reference photo â†’ generate each scene (your brief drives them) â†’ add background music and stereo if you like. Page 13 â†’ SYNC ALL TRACKS. Page 15 â†’ set the mix. Page 16 â†’ choose quality â†’ render. Page 17 â†’ preview. Page 18 â†’ export and share. That is a finished film, made by you, at mandastrong01.bolt.host."},
  ];
  return(
    <div style={{padding:"20px 32px 40px",maxWidth:860,margin:"0 auto"}}>
      <div style={{color:GOLD,fontWeight:900,fontSize:12,letterSpacing:4,marginBottom:12,textAlign:"center"}}>ðŸ“– HOW TO USE MANDASTRONG STUDIO â€” CLICK ANY SECTION</div>
      {SECTIONS.map((g,i)=>{
        const isOpen=open===i;
        return(
          <div key={i} style={{marginBottom:4}}>
            <button onClick={()=>setOpen(isOpen?null:i)} style={{width:"100%",background:isOpen?GOLD+"14":"#030200",border:"1px solid "+(isOpen?GOLD:GOLDDIM+"55"),padding:"13px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif"}}>
              ...
              <span style={{color:GOLD,fontSize:16,fontWeight:900}}>{isOpen?"â–²":"â–¼"}</span>
            </button>
...
function P24CharacterStudio({ onSave, go }) {
  ...
  const persist=(list)=>{
    setChars(list);
    try{localStorage.setItem("ms_characters",JSON.stringify(list));}catch(e){alert("Storage full â€” remove a character photo and try again.");}
  };
  ...
  const useInScene=(c)=>{
    if(onSave&&c.photo){
      ...
      alert("âœ“ "+c.name+" reference image sent to your Media Library â€” upload it on Page 8 to keep this character consistent.");
    }else{
  ...
  const generatePrompt=(c)=>{
    ...
    navigator.clipboard.writeText(prompt).then(()=>alert("âœ“ Scene prompt copied â€” paste into Page 8 scene description.")).catch(()=>alert(prompt));
  };
      ...
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
          ...
          <div style={{...Card(),border:"2px solid "+GOLD,maxHeight:"80vh",overflowY:"auto"}}>
            <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:14}}>{editId?"âœ EDIT CHARACTER":"âœ¦ CREATE A CHARACTER"}</div>
            ...
            {photo?(
              <div style={{position:"relative",marginBottom:12}}>
                ...
                <button onClick={()=>{setPhoto(null);setPhotoName("");}} style={{position:"absolute",top:5,right:5,background:"#000",border:"1px solid "+GOLD,color:GOLD,padding:"2px 8px",cursor:"pointer",fontSize:11,fontWeight:900}}>âœ•</button>
              </div>
            ):(
              <div
                ...
                <div>ðŸ–¼ DRAG & DROP A PHOTO HERE</div>
            ...
            <select value={voice} onChange={e=>setVoice(e.target.value)} style={{...inp,marginBottom:14,cursor:"pointer"}}>
              {VOICE_CHARACTERS.map(v=><option key={v.id} value={v.id} style={{background:"#000"}}>{v.name} â€” {v.origin} {v.gender} Â· {v.style}</option>)}
            </select>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button onClick={addChar} style={{...G("gold",false),padding:"14px",fontSize:13,letterSpacing:2}}>{editId?"âœ“ UPDATE":"+ SAVE CHARACTER"}</button>
              {editId&&<button onClick={clearForm} style={{...G("out",false),padding:"14px",fontSize:13,letterSpacing:2}}>CANCEL</button>}
            </div>
            {savedNote&&<div style={{marginTop:10,background:"#061406",border:"1px solid #22c55e",padding:"10px",textAlign:"center",color:"#22c55e",fontWeight:900,fontSize:12,letterSpacing:2}}>âœ“ CHARACTER SAVED</div>}
          </div>

          {/* Library panel */}
          <div>
            <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:14}}>YOUR CHARACTERS â€” {chars.length}</div>
            {chars.length===0?(
              <div style={{...Card(),textAlign:"center",padding:"40px 20px",color:GOLDDIM}}>
                <div style={{fontSize:34,marginBottom:10}}>ðŸŽ­</div>
                ...
                {chars.map(c=>(
                  <div key={c.id} style={{...Card(),padding:14}}>
                    <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                      {c.photo?<img src={c.photo} alt={c.name} style={{width:80,height:80,objectFit:"cover",border:"2px solid "+GOLD,flexShrink:0}}/>:<div style={{width:80,height:80,background:"#000",border:"1px solid "+GOLDDIM,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>ðŸŽ­</div>}
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {c.costume&&<div style={{color:WHITE,fontSize:11,marginTop:4,fontStyle:"italic"}}>ðŸ‘— {c.costume}</div>}
                        {c.personality&&<div style={{color:WHITE,fontSize:11,marginTop:2}}>ðŸ’­ {c.personality}</div>}
                        {c.notes&&<div style={{color:DIM,fontSize:11,marginTop:2,fontStyle:"italic"}}>{c.notes}</div>}
                        <div style={{color:WHITE,fontSize:11,marginTop:4}}>ðŸŽ™ {VOICE_CHARACTERS.find(v=>v.id===c.voice)?.name||c.voice}</div>
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5,marginTop:10}}>
                      <button onClick={()=>useInScene(c)} style={{background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",border:"none",color:"#000",padding:"7px 4px",cursor:"pointer",fontSize:10,fontWeight:900,letterSpacing:1,fontFamily:"'Rajdhani',sans-serif"}}>â†’ USE IN SCENE</button>
                      <button onClick={()=>generatePrompt(c)} style={{background:"transparent",border:"1px solid "+GOLD,color:GOLD,padding:"7px 4px",cursor:"pointer",fontSize:10,fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>ðŸ“‹ COPY PROMPT</button>
                      <button onClick={()=>editChar(c)} style={{background:"transparent",border:"1px solid "+GOLDDIM,color:WHITE,padding:"7px 4px",cursor:"pointer",fontSize:10,fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>âœ EDIT</button>
                      <button onClick={()=>removeChar(c.id)} style={{background:"none",border:"1px solid #ef4444",color:"#ef4444",padding:"7px 4px",cursor:"pointer",fontSize:10,fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>âœ• DELETE</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",marginTop:30,paddingBottom:20}}>
          <button onClick={()=>go&&go(1)} style={{...G("gold",false),padding:"14px 40px",fontSize:13,letterSpacing:3}}>ðŸ  HOME</button>
          <button onClick={()=>{try{localStorage.removeItem("ms_user");}catch{}window.location.reload();}} style={{...G("out",false),padding:"14px 40px",fontSize:13,letterSpacing:3}}>ðŸšª EXIT APP</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{position:"relative",zIndex:1,padding:"30px 24px 80px"}}>
        <div style={{maxWidth:880,margin:"0 auto",textAlign:"center"}}>
          <div style={{width:"100%",maxHeight:"34vh",overflow:"hidden",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",background:"#000",border:"1px solid "+GOLDDIM,marginBottom:26}}>
            ...
            {vidNeedsTap&&(
              <button onClick={tapPlayVideo} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.45)",border:"none",color:GOLD,fontSize:16,fontWeight:900,letterSpacing:3,cursor:"pointer",fontFamily:"'Cinzel',serif",display:"flex",alignItems:"center",justifyContent:"center"}}>â–¶ TAP TO PLAY</button>
            )}
          </div>
          <div style={{fontSize:10,color:GOLD,letterSpacing:6,marginBottom:8,fontWeight:700}}>MANDASTRONG STUDIO Â· CINEMA INTELLIGENCE PLATFORM</div>
          ...
          <div style={{...Card(),textAlign:"left",marginBottom:28,background:"#050500ee",border:"2px solid "+GOLD}}>
            <div style={{color:GOLD,fontWeight:900,fontSize:14,letterSpacing:3,marginBottom:16,textAlign:"center"}}>âœ¦ A SPECIAL THANK YOU âœ¦</div>
            ...
            <p style={{color:WHITE,fontSize:14,lineHeight:2,margin:0}}>Together, we are building a community of creators who use their talents to spread kindness, understanding, and hope. â€” <strong style={{color:GOLD}}>Amanda</strong></p>
          </div>
          <div style={{...Card(),textAlign:"left",marginBottom:28,background:"#030300ee",border:"1px solid "+GOLDDIM}}>
            <div style={{color:GOLD,fontWeight:900,fontSize:13,letterSpacing:3,marginBottom:12,textAlign:"center"}}>âœ¦ OUR MISSION âœ¦</div>
            ...
            <p style={{color:WHITE,fontSize:13,lineHeight:1.9,margin:0}}>All proceeds from <strong style={{color:GOLD}}>MandaStrong1.Etsy.com</strong> are donated directly to humanitarian causes â€” veterans mental health, anti-bullying programmes in schools, and children in need.</p>
          </div>
          <button onClick={()=>setHowOpen(o=>!o)} style={{width:"100%",background:howOpen?"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")":"#050500ee",border:"2px solid "+GOLD,color:howOpen?"#000":GOLD,padding:"18px 24px",cursor:"pointer",fontFamily:"'Cinzel',serif",fontSize:15,fontWeight:900,letterSpacing:4,marginBottom:howOpen?0:28,display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 0 30px "+GOLD+"44"}}>
            <span>ðŸ“– MANDASTRONG STUDIO â€” THE COMPLETE GUIDE &amp; AI HANDBOOK</span>
            <span style={{fontSize:18}}>{howOpen?"â–²":"â–¼"}</span>
          </button>
          {howOpen&&<div style={{background:"#030200ee",border:"2px solid "+GOLD,borderTop:"none",marginBottom:28}}><HowToGuide/></div>}
          <a href="https://MandaStrong1.Etsy.com" target="_blank" rel="noreferrer" style={{display:"block",background:"linear-gradient(135deg,"+GOLDDIM+","+GOLD+")",color:"#000",padding:"16px 24px",fontWeight:900,fontSize:14,letterSpacing:3,textDecoration:"none",marginBottom:28,fontFamily:"'Cinzel',serif"}}>ðŸ“š HUMANITY FOR FUTURE AI â€” MANDASTRONG1.ETSY.COM</a>
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={()=>{try{localStorage.setItem("ms_last_saved",new Date().toISOString());}catch{} setSavedMsg(true); setTimeout(()=>setSavedMsg(false),2500);}} style={{...G("gold",false),padding:"14px 40px",fontSize:13,letterSpacing:3}}>ðŸ’¾ SAVE</button>
            <button onClick={()=>go(24)} style={{...G("out",false),padding:"14px 40px",fontSize:13,letterSpacing:3}}>NEXT â†’</button>
          </div>
          {savedMsg&&<div style={{color:"#22c55e",fontSize:12,fontWeight:900,letterSpacing:2,marginTop:12}}>âœ“ PROJECT SAVED</div>}
        </div>
      </div>
    </div>
  );
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// CINEMATIC INTRO â€” black shiny gold doors that open to reveal the app
...
function IntroDoors({ onEnter }){
  ...
  const playChime=()=>{
    try{
      ...
      // 1) DEEP IMPACT BOOM â€” the off-guard hit
      ...
      // 2) SUB DRONE BED â€” tension underneath
      ...
      // 3) RISING MINOR STAB LINE â€” drama building to the doors
      ...
      // 5) RESOLVING HERO HIT â€” lands immediately AS the doors open (t=0)
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:100000,background:"#000",overflow:"hidden",
      ...
      <div style={{position:"absolute",left:0,right:0,bottom:"6%",display:"flex",flexDirection:"column",alignItems:"center",
        ...
        <div style={{color:GOLDDIM,fontSize:"clamp(8px,1.4vw,11px)",letterSpacing:3,marginTop:12,textAlign:"center",padding:"0 16px"}}>CINEMA INTELLIGENCE PLATFORM Â· 600+ AI TOOLS Â· UP TO 3-HOUR FILMS</div>
...
export default function App() {
  ...
  // â”€â”€ CINEMATIC INTRO â€” gold doors open to reveal the app â”€â”€
  const [showIntro,setShowIntro]=useState(true);
  const [menu,setMenu]=useState(false);
  useEffect(()=>{
    // Raise the storage ceiling so large uploads don't crash â€” ask the browser
    ...
    // Viewport â€” responsive for all devices
    ...
    // PWA MANIFEST â€” makes the DOWNLOAD APP button work as a real install
    try{
      const manifestData={
        name:"MandaStrong Studio",
        short_name:"MandaStrong",
        description:"Cinema Intelligence Platform â€” 600+ AI tools, 24 pages, up to 3-hour films",
        start_url:"/",
        display:"standalone",
  ...
  // Real auto-persist â€” saves state silently whenever page, timeline or mediaLib changes
  useEffect(()=>{
  ...
  // Emergency crash save â€” fires when tab is closed or crashes
  useEffect(()=>{
    const emergencySave=()=>{
      try{
  ...
  const renderPage=()=>{
    switch(page){
      ...
      case 5: return <ToolPage title="WRITING TOOLS" subtitle="AI WORKSTATION 01 â€” WRITING" tools={WRITING} onSave={saveAsset}/>;
      case 6: return <P6Voice onSave={saveAsset} setMediaLib={setMediaLib}/>;
      case 7: return <ToolPage title="IMAGE TOOLS" subtitle="AI WORKSTATION 03 â€” IMAGE" tools={IMAGE_T} onSave={saveAsset}/>;
      case 8: return <P8VideoGenerator onSave={saveAsset} user={user} filmDuration={filmDuration} setFilmDuration={setFilmDuration}/>;
      case 9: return <ToolPage title="MOTION & VFX" subtitle="AI WORKSTATION 05 â€” MOTION" tools={MOTION} onSave={saveAsset}/>;
      case 10: return <ToolPage title="ENHANCEMENT STUDIO" subtitle="AI WORKSTATION 06 â€” ENHANCE" tools={MOTION} onSave={saveAsset}/>;
  ...
  return (
    <div style={{background:"#000",minHeight:"100vh",fontFamily:"'Rajdhani',sans-serif"}}>
      ...
      {savedNotice&&<div style={{position:"fixed",top:60,left:"50%",transform:"translateX(-50%)",background:GOLDDIM,color:"#000",padding:"10px 24px",fontWeight:900,fontSize:13,letterSpacing:2,zIndex:999}}>âœ“ PROJECT SAVED</div>}

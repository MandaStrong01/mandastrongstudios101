...
async function proxyFetch(body){
  const res=await fetch("https://njqfexhltjwpgvctmyaw.supabase.co/functions/v1/claude-proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
  return res.json();
}
...
function MusicVideoStudio({ onClose, onSave }) {
  ...
  const [config,setConfig]=useState({
    title:"If Only",artist:"MandaStrong",genre:"Folk / Acoustic",mood:"Emotional",
    colorGrade:"Warm Gold",videoStyle:"Cinematic Narrative",
    visualDesc:"Night. A man sits on a windowsill, back to camera, face never shown. Ocean outside. Full moon. Single candle on the sill. Guitar in his hands.",
    duration:3
  });
  const audioInputRef=useRef(null);
  const GRADES=["Warm Gold","Teal & Orange","Cold Blue","Desaturated","High Contrast","Warm Amber"];
  const STYLES=["Cinematic Narrative","Lyric Video","Performance","Abstract","Nature Cuts","Urban Night"];
  ...
  const MOODS=["Euphoric","Melancholic","Tense","Romantic","Hopeful","Dark","Energetic","Peaceful","Nostalgic","Angry"];
  const addLog=(m)=>setRenderLog(p=>[...p,m]);

  const handleAudioUpload=(e)=>{
    const f=e.target.files&&e.target.files[0];
    if(!f)return;
    setAudioFile(f);setAudioName(f.name);setAudioUrl(URL.createObjectURL(f));
    addLog("Audio loaded: "+f.name);
  };

  const generateMusicVideo=async()=>{
    if(!audioUrl){alert("Upload your audio track first");return;}
    setGenerating(true);setRenderProgress(0);setRenderLog([]);setVideoUrl("");
    addLog("MandaStrong Cinema Engine — reading your scene...");
    setRenderProgress(10);
    try{
      const totalSec=(config.duration||3)*60;
      const sceneDesc=config.visualDesc||"Cinematic music video scene";
      const filmPrompt=`You are the MandaStrong Cinema Engine. Write a photorealistic JavaScript canvas function for a music video.
...
Write: function renderFilm(ctx, W, H, t, sec, totalSec, beatNow)
Where: W=1920 H=1080 t=0-1 sec=elapsed beatNow=true on beat frames

RULES: ALL gradients, ZERO flat fills, ZERO cartoon outlines. Real human figures with radialGradient skin tones. Cinematic lighting. Return ONLY the function.`;
      ...
      let renderFn;
      try{renderFn=new Function("ctx","W","H","t","sec","totalSec","beatNow",bO>=0&&bC>bO?code.slice(bO+1,bC):"ctx.fillStyle='#020100';ctx.fillRect(0,0,W,H);");}
      catch(e){addLog("Compile error — using fallback");renderFn=(ctx,W,H,t,sec)=>{const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,"rgb(1,2,12)");g.addColorStop(1,"rgb(8,18,55)");ctx.fillStyle=g;ctx.fillRect(0,0,W,H);};}

      addLog("Renderer ready — capturing...");setRenderProgress(25);

      const canvas=canvasRef.current;canvas.width=1920;canvas.height=1080;
      const ctx=canvas.getContext("2d");
      const fps=24,dur=totalSec;
      const totalFrames=fps*dur;

      // Set up audio
      let audioCtx,audioDest,gainNode;
      try{
      ...
      }catch(e){addLog("Audio setup: "+e.message);}

      const mimeType=MediaRecorder.isTypeSupported("video/webm;codecs=vp9")?"video/webm;codecs=vp9":"video/webm";
      const streams=[canvas.captureStream(fps)];
      if(audioDest)streams.push(audioDest.stream);
      const combined=audioDest
        ?new MediaStream([...canvas.captureStream(fps).getTracks(),...audioDest.stream.getTracks()])
        :canvas.captureStream(fps);
      const recorder=new MediaRecorder(combined,{mimeType,videoBitsPerSecond:15000000});
      const chunks=[];
      recorder.ondataavailable=e=>{if(e.data.size>0)chunks.push(e.data);};
      recorder.start(Math.round(1000/fps));

      await new Promise(resolve=>{
        let frame=0;const startTime=performance.now();const msPerFrame=Math.round(1000/fps);
        const tick=()=>{
          if(frame>=totalFrames){resolve(null);return;}
          const t=frame/totalFrames,sec=frame/fps;
          const beatNow=frame%Math.round(fps*0.5)===0;
          try{
            ctx.clearRect(0,0,1920,1080);
            renderFn(ctx,1920,1080,t,sec,totalSec,beatNow);
            const vg=ctx.createRadialGradient(960,540,200,960,540,1100);vg.addColorStop(0,"rgba(0,0,0,0)");vg.addColorStop(1,"rgba(0,0,0,0.85)");ctx.fillStyle=vg;ctx.fillRect(0,0,1920,1080);
            ...
            if(t>0.93){ctx.fillStyle="rgba(0,0,0,"+((t-0.93)/0.07)+")";ctx.fillRect(0,0,1920,1080);}
          }catch(e){ctx.fillStyle="#020100";ctx.fillRect(0,0,1920,1080);}
          setRenderProgress(25+Math.round((frame/totalFrames)*70));
          if(frame%fps===0)addLog("  "+Math.round(sec)+"s / "+dur+"s");
          frame++;
          const due=startTime+(frame*msPerFrame);
          setTimeout(tick,Math.max(4,due-performance.now()));
        };tick();
      });

      setRenderProgress(97);addLog("Finalising...");
      ...
      addLog("✓ Music video complete — "+(blob.size/1024/1024).toFixed(1)+"MB");
      if(onSave)onSave({id:"mv_"+Date.now(),name:config.title+"_MusicVideo.webm",type:"video/webm",url,file:new File([blob],config.title+"_MusicVideo.webm",{type:"video/webm"})});
      setStep(4);
    }catch(e){addLog("Error: "+e.message);}
    setGenerating(false);
  };

  const lbl=(t)=><div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:2,marginBottom:6}}>{t}</div>;
  const sel=(key,opts)=><select value={config[key]} onChange={e=>setConfig(p=>({...p,[key]:e.target.value}))}
    style={{width:"100%",background:"#0a0a0a",border:`1px solid ${GOLDDIM}`,color:WHITE,padding:"8px 10px",fontSize:12,marginBottom:10,outline:"none",fontFamily:"'Rajdhani',sans-serif"}}>
    {opts.map(o=><option key={o} value={o} style={{background:"#000"}}>{o}</option>)}</select>;

  return(
    <div style={{position:"fixed",inset:0,zIndex:1100,background:"rgba(0,0,0,0.96)",display:"flex",flexDirection:"column",fontFamily:"'Rajdhani',sans-serif"}}>
      <canvas ref={canvasRef} style={{display:"none"}}/>
      <div style={{background:"linear-gradient(135deg,#0a0500,#000)",borderBottom:`2px solid ${GOLD}`,padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div>
          <div style={{fontSize:10,color:GOLD,letterSpacing:4,fontWeight:700}}>MANDASTRONG STUDIO · MUSIC VIDEO STUDIO</div>
          <h2 style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:22,fontWeight:900,letterSpacing:4,margin:0}}>MUSIC VIDEO STUDIO</h2>
        </div>
        <button onClick={onClose} style={{background:"none",border:`1px solid ${GOLDDIM}`,color:GOLD,padding:"6px 18px",cursor:"pointer",fontSize:12,fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>✕ CLOSE</button>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:20,display:"grid",gridTemplateColumns:"1fr 380px",gap:20}}>
        <div>
          {/* STEP 1: AUDIO */}
          <div style={{...Card(),marginBottom:14}}>
            <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:3,marginBottom:10}}>STEP 1 — UPLOAD YOUR AUDIO TRACK</div>
            <input ref={audioInputRef} type="file" accept="audio/*" style={{display:"none"}} onChange={handleAudioUpload}/>
            {audioUrl?(
              <div>
                <div style={{color:"#22c55e",fontSize:11,fontWeight:900,letterSpacing:2,marginBottom:8}}>✓ {audioName}</div>
                ...
                <button onClick={()=>audioInputRef.current&&audioInputRef.current.click()}
                  style={{...G("out",true),fontSize:10}}>CHANGE TRACK</button>
              </div>
            ):(
              <div onClick={()=>audioInputRef.current&&audioInputRef.current.click()}
                style={{border:`2px dashed ${GOLDDIM}`,padding:"24px",textAlign:"center",cursor:"pointer"}}
                onMouseEnter={e=>e.currentTarget.style.borderColor=GOLD}
                onMouseLeave={e=>e.currentTarget.style.borderColor=GOLDDIM}>
                <div style={{color:GOLD,fontSize:13,fontWeight:900,marginBottom:4}}>⬆ CLICK TO UPLOAD AUDIO</div>
                <div style={{color:DIM,fontSize:10}}>MP3 · WAV · M4A · AAC</div>
              </div>
            )}
          </div>
          ...
          {/* STEP 2: CONFIG */}
          <div style={{...Card(),marginBottom:14}}>
            <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:3,marginBottom:10}}>STEP 2 — CONFIGURE YOUR VIDEO</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div>
                {lbl("TITLE")}
                <input value={config.title} onChange={e=>setConfig(p=>({...p,title:e.target.value}))}
                  style={{width:"100%",background:"#0a0a0a",border:`1px solid ${GOLDDIM}`,color:WHITE,padding:"8px 10px",fontSize:12,marginBottom:10,outline:"none",boxSizing:"border-box",fontFamily:"'Rajdhani',sans-serif"}}/>
              </div>
              <div>
                {lbl("ARTIST")}
                <input value={config.artist} onChange={e=>setConfig(p=>({...p,artist:e.target.value}))}
                  style={{width:"100%",background:"#0a0a0a",border:`1px solid ${GOLDDIM}`,color:WHITE,padding:"8px 10px",fontSize:12,marginBottom:10,outline:"none",boxSizing:"border-box",fontFamily:"'Rajdhani',sans-serif"}}/>
              </div>
            </div>
            {lbl("GENRE")}{sel("genre",GENRES)}
            {lbl("MOOD")}{sel("mood",MOODS)}
            {lbl("COLOUR GRADE")}{sel("colorGrade",GRADES)}
            {lbl("VIDEO STYLE")}{sel("videoStyle",STYLES)}
            <div style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                {lbl("DURATION")}
                <span style={{color:WHITE,fontSize:11,fontWeight:900}}>{config.duration} MIN</span>
              </div>
              <input type="range" min={1} max={10} step={1} value={config.duration}
                onChange={e=>setConfig(p=>({...p,duration:+e.target.value}))}
                style={{width:"100%",accentColor:GOLD}}/>
              <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
                <span style={{color:DIM,fontSize:9}}>1 min</span>
                <span style={{color:DIM,fontSize:9}}>10 min</span>
              </div>
            </div>
          </div>
          ...
          {/* STEP 3: SCENE */}
          <div style={{...Card(),marginBottom:14}}>
            <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:3,marginBottom:10}}>STEP 3 — DESCRIBE YOUR SCENE</div>
            <div style={{color:DIM,fontSize:10,marginBottom:8,lineHeight:1.7}}>Describe what you want to see. The engine renders it photorealistically to your music.</div>
            <textarea value={config.visualDesc} onChange={e=>setConfig(p=>({...p,visualDesc:e.target.value}))}
              style={{width:"100%",background:"#0a0a0a",border:`1px solid ${GOLDDIM}`,color:WHITE,padding:"10px 12px",fontSize:12,outline:"none",boxSizing:"border-box",fontFamily:"'Rajdhani',sans-serif",lineHeight:1.8,height:110,resize:"none"}}/>
          </div>

          <button onClick={generateMusicVideo} disabled={generating||!audioUrl}
            style={{background:generating||!audioUrl?"#0a0800":`linear-gradient(135deg,${GOLDDIM},${GOLD})`,border:"none",color:generating||!audioUrl?GOLD:"#000",width:"100%",padding:"18px",fontSize:14,letterSpacing:3,cursor:generating||!audioUrl?"not-allowed":"pointer",fontWeight:900,fontFamily:"'Rajdhani',sans-serif"}}>
            {generating?"⟳ RENDERING MUSIC VIDEO... "+renderProgress+"%":"🎬 GENERATE MUSIC VIDEO"}
          </button>
        </div>

        {/* RIGHT PANEL */}
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{background:"#000",aspectRatio:"16/9",border:`1px solid ${GOLDDIM}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden"}}>
            {videoUrl?(
              <video src={videoUrl} controls autoPlay loop playsInline style={{width:"100%",height:"100%",objectFit:"contain"}}/>
            ):(
              <div style={{textAlign:"center",padding:20}}>
                <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:2,marginBottom:6}}>PREVIEW</div>
                <div style={{color:DIM,fontSize:10,lineHeight:1.8}}>Upload audio<br/>Configure your video<br/>Describe your scene<br/>Generate</div>
              </div>
            )}
          </div>
          {generating&&(
            <div style={{...Card()}}>
              <div style={{height:4,background:"#111",marginBottom:6}}>
                <div style={{width:renderProgress+"%",height:"100%",background:`linear-gradient(90deg,${GOLDDIM},${GOLD})`,transition:"width .3s"}}/>
              </div>
              <div style={{color:GOLD,fontSize:10,textAlign:"center",letterSpacing:2,marginBottom:8}}>{renderProgress}%</div>
            </div>
          )}
          {videoUrl&&!generating&&(
            <div style={{display:"flex",gap:8}}>
              <a href={videoUrl} download={config.title+"_MusicVideo.webm"}
                style={{flex:1,background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`,color:"#000",padding:"10px",fontSize:11,textDecoration:"none",textAlign:"center",fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif",display:"block"}}>⬇ DOWNLOAD</a>
              <button onClick={()=>{setVideoUrl("");setRenderLog([]);setRenderProgress(0);setStep(1);}}
                style={{flex:1,background:"transparent",border:`1px solid ${GOLD}`,color:GOLD,padding:"10px",fontSize:11,cursor:"pointer",fontWeight:900,letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>NEW VIDEO</button>
            </div>
          )}
          <div style={{...Card(),flex:1,overflowY:"auto",maxHeight:280}}>
            <div style={{color:GOLD,fontSize:10,fontWeight:900,letterSpacing:2,marginBottom:8}}>PRODUCTION LOG</div>
            {renderLog.length===0?(
              <div style={{color:DIM,fontSize:10,lineHeight:1.8}}>Log will appear here during rendering...</div>
            ):renderLog.map((l,i)=>(
              <div key={i} style={{color:i===renderLog.length-1?"#22c55e":DIM,fontSize:10,lineHeight:1.8,letterSpacing:1}}>{i===renderLog.length-1?"▶ ":"  "}{l}</div>
            ))}
          </div>
        </div>
      </div>
...
function P8VideoGenerator({ onSave, user, filmDuration, setFilmDuration }) {
  ...
  const RENDER_STYLES={
    photorealistic:{label:"📷 Photorealistic",desc:"Real humans, real environments, real lighting",rules:"PHOTOREALISTIC: real human skin tones radialGradient, real environments, all gradients no flat fills, zero cartoon outlines"},
    cinematic:{label:"🎬 Cinematic",desc:"Hollywood dramatic lighting",rules:"CINEMATIC: dramatic lighting, deep contrast, teal-orange grade, film grain, letterbox bars"},
    documentary:{label:"🎥 Documentary",desc:"Warm, natural, authentic",rules:"DOCUMENTARY: natural warm lighting, amber grade, authentic human moments, handheld feel"},
    noir:{label:"🌑 Film Noir",desc:"Black and white, deep shadows",rules:"FILM NOIR: near-monochrome, deep dramatic shadows, single harsh light source"},
    golden:{label:"🌅 Golden Hour",desc:"Warm golden sunlight",rules:"GOLDEN HOUR: warm amber gold lighting, sun near horizon, long shadows, dust particles"},
    scifi:{label:"🚀 Sci-Fi",desc:"Futuristic neon technology",rules:"SCI-FI: cold blue teal lighting, neon accents, technology surfaces, atmospheric haze"},
    horror:{label:"👻 Horror",desc:"Dark unsettling atmosphere",rules:"HORROR: very dark environment, single cold light, deep shadows, desaturated with green tinge"},
    animated:{label:"✨ Animated/Stylised",desc:"Bold colours, stylised beauty",rules:"STYLISED: bold beautiful colours, smooth gradients, stylised but NOT cartoon, no thick outlines"},
  };
  ...
  const GENRE_TEMPLATES={
    feature:["A lone detective walks rain-soaked city streets at night. Neon reflections. Deep shadows.","Two strangers meet on a train platform at golden hour. Steam rising.","A family stands in an empty house. Afternoon sun. Nobody speaking."],
    documentary:["Elderly woman in a small kitchen. Morning light from one window. Tea cooling. Hands folded.","Three children plant saplings at golden hour. Muddy hands, wide smiles.","A scientist in a dark lab at 3am. Single monitor light. Data that changes everything."],
    musicvideo:["Night. A man on a windowsill, back to camera. Ocean outside. Full moon. Guitar in hand.","Empty dance floor. One spotlight. A woman dances alone, eyes closed.","Rain on a window at night. City lights blurred. A figure sits still."],
    shortfilm:["A child leaves a letter under a door and runs. Morning light.","Two old friends on a park bench in autumn. Neither speaking.","A woman votes for the first time. Morning light. People watching."],
    horror:["Dark corridor. Single flickering bulb at the far end. Something moves in shadow.","An old house at night. Every window dark except one on the top floor.","Deep forest at dusk. Mist between the trees. A figure standing far off."],
    scifi:["Vast space station interior. Stars through a curved window. An astronaut floats alone.","City of the future at night. Towers of glass. Flying vehicles. Rain.","AI terminal glowing blue in a dark room. A scientist reads the output."],
    romance:["Two people share an umbrella in the rain. Neither moves away.","Rooftop at sunset. City below. Two cups of coffee going cold.","Morning light through curtains. Someone asleep. Complete tenderness."],
    thriller:["A woman checks the locks on her door. Three times. Then again.","Empty parking garage. Footsteps echo. A car that was not there before.","A man opens an envelope. Reads it. Reads it again. Sits very still."],
    action:["A fighter stands alone in a ring. Sweat and bruises. Breathing hard.","Car chase through narrow streets at night. Rain on the windscreen.","Rooftop. Two figures face each other. City below. Wind."],
    comedy:["A man tries to assemble furniture. The cat sits on the most important page.","First day of a new job. Wrong building. Wrong floor.","Someone attempts to cook a romantic dinner. Everything is slightly on fire."],
    drama:["Father and grown child at opposite ends of a table. Food going cold.","A hospital waiting room at 4am. One person. Fluorescent light.","Graduation day. Parents watching. Their child does not look for them."],
    animation:["A tiny boat on a vast sea of stars. The sailor does not know they are in space.","A forest where every tree glows from inside. A child walks through it.","An old clockmaker winds the last clock. The world outside begins to move."],
    historical:["A woman writes a letter by candlelight. 1942.","A soldier returns to a village after the war.","A coronation. Thousands watching. One person looks at the others."],
    nature:["Dawn on a mountain lake. Absolute stillness. First light touching the water.","Deep ocean. A whale passes slowly below. Silence.","A forest in winter. Everything white and still. Then a fox."],
  };
  ...
  const buildPrompt=(sceneDesc,refNote)=>{
    ...
    const styleRules=(RENDER_STYLES[renderStyle]||RENDER_STYLES.photorealistic).rules;
    const genreObj=genre?FILM_GENRES.find(g=>g.id===genre):null;
    const genreName=genreObj?genreObj.label:"";
    const jsonTmpl=JSON.stringify({timeOfDay:"night",skyTop:"1,2,12",skyMid:"6,14,45",skyBot:"10,22,65",hasStars:true,hasOcean:false,hasForest:false,hasCity:false,hasFire:false,hasRain:false,hasSnow:false,hasCandle:false,hasMoon:true,isIndoor:false,isSpace:false,isUnderwater:false,wallColor:"8,5,3",horizonY:0.54,gradeR:30,gradeG:15,gradeB:0,gradeA:0.06,textOverlay:"",characters:[{x:0.3,y:0.6,facingBack:false,seated:false,skinLight:"220,170,120",skinDark:"150,100,65",clothingTop:"60,40,20",clothingBot:"30,20,10",hairColor:"30,20,10",hasGuitar:false}],candleX:0.68,candleY:0.58});
    return ["You are a cinematographer. Return ONLY valid JSON for this scene.","LOCKED STYLE: "+styleName+" - "+styleRules,genreName?"GENRE: "+genreName:"","SCENE: "+sceneDesc,refNote||"","Return ONLY JSON matching this structure: "+jsonTmpl].filter(Boolean).join(" | ");
  };
...
function drawFrame(ctx, W, H, t, sec) {`;
      ...
      const totalFrames=Math.max(fps*5, Math.round((duration||30)*fps));
      const mimeType=MediaRecorder.isTypeSupported("video/webm;codecs=vp9")?"video/webm;codecs=vp9":"video/webm";
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 420px",minHeight:"calc(100vh - 120px)"}}>
        <div style={{padding:20,overflowY:"auto"}}>
          <div style={{marginBottom:12}}>
            ...
            {/* GENRE */}
            <div style={{marginBottom:12}}>
              <div style={{color:GOLD,fontSize:11,letterSpacing:3,fontWeight:900,marginBottom:6}}>GENRE & TEMPLATES</div>
              <select value={genre} onChange={e=>{setGenre(e.target.value);setShowTemplates(!!e.target.value);}}
                style={{width:"100%",background:"#0a0800",border:`1px solid ${GOLD}`,color:GOLD,padding:"9px 12px",fontSize:12,outline:"none",fontFamily:"'Rajdhani',sans-serif",cursor:"pointer",marginBottom:4}}>
              ...
              {showTemplates&&genre&&GENRE_TEMPLATES[genre]&&(
                <div style={{background:"#030200",border:`1px solid ${GOLDDIM}`,padding:8,marginTop:4}}>
                  {GENRE_TEMPLATES[genre].map((tmpl,i)=>(
                    <div key={i} onClick={()=>{setPrompt(tmpl);setShowTemplates(false);}}
                      style={{padding:"6px 8px",marginBottom:3,cursor:"pointer",fontSize:11,color:WHITE,lineHeight:1.6,border:`1px solid ${GOLDDIM}`}}
          ...
          <div style={{background:"#0a0a0a",border:`1px solid ${GOLDDIM}`,padding:14,marginBottom:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
              <span style={{color:DIM,fontSize:10}}>5s</span>
